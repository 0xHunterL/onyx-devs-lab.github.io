...
截至 2026 年 9 月 13 日。每一级只陈述已经保存的证据，不从爬虫访问推断收录、引用或推荐。
...
GPTBot 已验证正文请求 28 次；所有已核验爬虫合计覆盖 45 个规范正文路径。Wayback 公开 CDX 另记录 94 条成功 HTML 捕获，覆盖 44 个 URL；固定提示词的 23 个唯一证据页中已有 16 页归档，12 条提示词的全部证据页均已归档。23 条固定提示词中 18 条至少有一个证据页被核验抓取、12 条全部证据页被抓取；搜索／检索相关爬虫覆盖为 18/23。

部分已验证

### 公开搜索检索

Yandex 精确品牌查询已返回 Onyx 官方主域，且该页面的规范地址指向香港站；香港子域深页和非品牌类目检索仍未验证。

尚未验证

### AI 检索和引用

尚未记录豆包或其他 AI 在不提供网址时召回并引用 Onyx 页面。
