...
- [Onyx GEO monitor evidence checkpoint — 2026-09-13, revision 74](https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-74) — immutable provider-maintained copies of the monitor snapshot, public status, prompt matrix, evidence map, and crawler-coverage summary. It preserves the direct Bing exact-brand observation that showed the official apex and both Hong Kong localized homepages in the first visible results, alongside the stale legacy-apex snippet and negative category-query boundaries. It also records a provider-verified AhrefsBot `/sitemap.xml` request at `09:49:30Z`, bringing Ahrefs discovery-file requests to 106 while content requests remain zero. These observations do not prove exhaustive indexing across all regions or pages, stable ranking, Yep, Copilot or other AI retrieval, citation, recommendation, or independent endorsement; no prompt was sent to Doubao. The [tracked live status](https://hk.onyxdevslab.com/data/ai-search-evidence-status.json?utm_source=github_releases&utm_medium=referral&utm_campaign=geo_monitor_evidence_74) remains mutable.
