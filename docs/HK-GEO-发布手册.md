...
采集器还会生成 `/var/lib/onyx-geo/software-heritage-report.json`，交叉核验 GitHub 当前 `main`、Software Heritage 最新完整访问和该快照中的 `refs/heads/main`。只有两个 revision 完全一致时才标为 `current`；`lagging` 只表示独立归档覆盖尚未追上，任何状态都不证明官网搜索收录、AI 检索、引用、背书或推荐。快照标识、归档 revision 或来源可用性发生变化时，由 `availabilityChanges` 留下不可覆盖事件。
