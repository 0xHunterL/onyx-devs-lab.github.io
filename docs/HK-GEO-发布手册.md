...
采集器还会生成 `/var/lib/onyx-geo/software-heritage-report.json`，交叉核验 GitHub 当前 `main`、Software Heritage 最新完整访问和该快照中的 `refs/heads/main`。它同时跟踪当前版本化基线记录的 Save Code Now 请求 ID，将请求、任务、访问状态、快照标识和提供方下一次重试时间写入报告；这些字段变化也会由 `availabilityChanges` 留下不可覆盖事件。只有完整快照中的 revision 与 GitHub `main` 完全一致时才标为 `current`；请求被接受、安排重试或单独返回 `snapshot_swhid` 都不能归档成功。`lagging` 只表示独立归档覆盖尚未追上，任何状态都不证明官网搜索收录、AI 检索、引用、背书或推荐。

Wayback 查询可用时，捕获数减少也不能自动解释成快照或页面消失。监测应以追加事件和已见指纹保留曾经返回的记录，把当前 CDX 数量如实发布为“本次公开索引结果”，并在独立复测后记录索引回撤／去重的解释边界。

采集成功后，`geo:check-evidence-publication-drift` 会将生产汇总中的监测完整性、搜索相关提示词覆盖与精确证据页集合、归因和站外可用性与仓库当前版本化基线逐项比较，并原子写入 `/var/lib/onyx-geo/publication-drift.json`。提供方已核验爬虫的累计计数按已发布下界校验：新增请求由追加事件留痕，不再仅因数字自然增长制造新 Release；计数回退、正文路径或提示词覆盖集合改变仍产生 `drift`。任何必需的爬虫身份核验源不可用或从采集覆盖中消失也必须产生 `drift`。Common Crawl 与 Wayback 是有日期的外部快照，结果会随索引轮换、去重或短暂收缩；这些变化继续写入摘要和不可覆盖事件，但不单独阻断服务，且来源不可用时绝不能用占位零覆盖上次可用证据。归因分别比较最新已验证站外引荐和最新两条访客类型未核实记录，站外分发按当前版本化清单逐项比较 `itemId`、来源类型和完整 URL，不能用相同总数掩盖 campaign、归因落地页、抓取证据页、Release 或目标 URL 的集合替换。`retentionAdjustments` 只说明某一轮当前保留日志是否需要使用追加账本抬高累计计数，会随日志窗口自然出现或消失；门禁独立验证连续轮转谱系，不把这个瞬时诊断数组本身当作外部证据漂移。归因记录仅保留监测所需的时间、来源、引荐主机、campaign、落地页和状态，不写入 IP 或 User-Agent。状态为 `drift` 时 systemd 服务显式失败，防止真正的结构变化静默发生；该失败仅表示需要人工审核，不代表可发现性下降，也不会自动发布未经复核的日志。
...
GitHub 发布工作流在部署前运行 `geo:test`，当前共 312 项，覆盖站点生成、监测摘要、爬虫身份与路径分类、引荐自动化分类、Wayback CDX、Software Heritage 最新完整快照与 GitHub `main` 对齐判断、固定提示词归档覆盖解析、日志轮转后的已核验指纹累计，以及发布漂移的同步、故障、完整爬虫计数、归因结构、连续编号轮转谱系、累计差异和不可用索引标识完整性样例。自然增加或到期的连续轮转文件不会制造发布漂移，但缺号、意外路径、重复文件或当前日志缺失仍会失败。任何回归都会阻止 Pages 部署；测试通过只证明这些分类和门禁按固定样例工作，不证明外部平台已经抓取、收录或引用。
