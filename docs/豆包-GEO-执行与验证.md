...
- 服务使用 Service；指南与案例使用 Article；团队页使用 AboutPage、ProfilePage、Person 和 Organization 结构化数据。文章及案例的 `datePublished` 按 Git 首次公开记录填写，并在正文显示与 Schema 一致的组织作者、发布和更新日期；三语作者链接分别进入对应的 Organization ProfilePage，而不是用统一日期或隐藏署名制造虚假新鲜度。实现依据 [Google Article 结构化数据规范](https://developers.google.com/search/docs/appearance/structured-data/article) 与[署名日期指南](https://developers.google.com/search/docs/appearance/publication-dates)。
