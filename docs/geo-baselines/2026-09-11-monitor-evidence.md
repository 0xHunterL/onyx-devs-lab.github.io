...
- 依据 Yandex Webmaster 官方方法，对日志中每个 YandexBot 地址执行 PTR 反查，要求主机名以 `.yandex.ru`、`.yandex.net` 或 `.yandex.com` 结尾，并正向解析回原地址。当前及数字轮转日志共识别 70 次已核验正文请求和 64 次发现文件请求，时间范围为 `2026-09-08T12:45:20Z` 至 `2026-09-11T15:15:15Z`，YandexBot 自身覆盖 41 个正文路径。前 132 次属于补齐监测后识别出的历史证据，最新两次才是新发生的抓取；它证明 Yandex 搜索爬虫访问，不证明公开收录、排名、AI 检索或引用。
