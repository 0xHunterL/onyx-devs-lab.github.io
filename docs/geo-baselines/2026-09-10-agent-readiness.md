...
- HTML and Markdown responses expose the Sitemap, Atom Feed, and `llms.txt` through RFC 8288 `Link` headers.
...
- IndexNow accepted a submission containing 62 canonical HTML URLs and 9 same-host machine resources with HTTP `200`.
...
## Versioned machine-readable asset
...
The SHA-256 above fixes the exact bytes uploaded with this checkpoint. Later live status versions may legitimately differ and should receive a new dated checkpoint rather than silently replacing this asset.
