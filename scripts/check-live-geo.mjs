...
for (const [pathname, body] of [['/llms.txt', llms.body], ['/llms-full.txt', llmsFull.body], ['/feed.xml', feed.body], ['/feed.json', jsonFeedResponse.body]]) {
  ...
  if (!body.includes('https://archive.softwareheritage.org/swh:1:snp:947880d501d459884fefdaf1bc95a9978599727a/')) failures.push(`${pathname}: Software Heritage snapshot is missing`);
  ...
  for (const page of ['ai-consulting/', 'ai-custom-development/', 'forward-deployed-engineering/', 'ai-search-geo-evidence/']) if (!body.includes(`https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/${page}`)) failures.push(`${pathname}: focused buyer-guide page is missing: ${page}`);
  ...
  if (!body.includes('https://github.com/mixuechu/hong-kong-enterprise-ai-buyers-guide/releases/tag/buyers-guide-geo-evidence-2026-09-11')) failures.push(`${pathname}: versioned buyer-guide checkpoint is missing`);
...
try {
  ...
  if (!organization.subjectOf?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-11-2' && item.sameAs === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-11-2/2026-09-11-monitor-evidence.json')) failures.push('/data/organization.json: automated GEO monitor evidence relation is missing');
  if (!organization.subjectOf?.some((item) => item.identifier === 'swh:1:snp:947880d501d459884fefdaf1bc95a9978599727a' && item.version === 'fe91aae7bb44331aac110650d1af4cfebf6364d3')) failures.push('/data/organization.json: Software Heritage snapshot relation is missing');
  ...
  if (!organization.subjectOf?.some((item) => item.url === 'https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/' && item.sameAs === 'https://github.com/mixuechu/hong-kong-enterprise-ai-buyers-guide/releases/tag/buyers-guide-geo-evidence-2026-09-11' && item.hasPart?.length === 4 && item.hasPart.some((part) => part.url === 'https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/ai-search-geo-evidence/') && item.hasPart.every((part) => part.url?.startsWith('https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/')))) failures.push('/data/organization.json: focused buyer-guide relations are missing');
...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== '2026.09.11.2' || status.observedAt !== '2026-09-11T06:22:24Z') failures.push('/data/ai-search-evidence-status.json: unexpected schema, version, or observation time');
  if (status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 7) failures.push('/data/ai-search-evidence-status.json: current OAI-SearchBot discovery count is stale');
  if (status.sameAs !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-11-2/2026-09-11-monitor-evidence.json') failures.push('/data/ai-search-evidence-status.json: current versioned release asset is missing');
  ...
  if (status.evidenceLevels?.[1]?.evidence?.verifiedGptBotContentCrawls !== 24 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 7 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotContentCrawls !== 0 || status.evidenceLevels?.[1]?.evidence?.historicallyVerifiedBingbotContentCrawls !== 7) failures.push('/data/ai-search-evidence-status.json: crawler evidence is incomplete');
...
for (let index = 0; index < urls.length; index += 8) {
  await Promise.all(urls.slice(index, index + 8).map(async (absoluteUrl) => {
    ...
    if (!page.body.includes('"identifier":"swh:1:snp:947880d501d459884fefdaf1bc95a9978599727a"')) failures.push(`${url.pathname}: Software Heritage snapshot reference is missing`);
