...
try {
  ...
  if (currentPublicSearchRetest.nonBrandCategoryResultsObserved?.length !== 11 || !currentPublicSearchRetest.nonBrandCategoryResultsObserved.some((item) => item.url === 'https://www.fde.hk/zh-hk') || !currentPublicSearchRetest.nonBrandCategoryMeaning?.includes('not a complete ranking')) failures.push('/data/ai-search-evidence-status.json: non-brand category result baseline is missing or misleading');
