...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== '2026.09.11.7' || status.observedAt !== '2026-09-11T09:12:38Z') failures.push('/data/ai-search-evidence-status.json: unexpected schema, version, or observation time');
  ...
  if (status.sameAs !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-11-7/2026-09-11-monitor-evidence.json') failures.push('/data/ai-search-evidence-status.json: current versioned release asset is missing');
  ...
  if (status.evidenceLevels?.[1]?.evidence?.verifiedGptBotContentCrawls !== 27 || status.evidenceLevels?.[1]?.evidence?.verifiedGptBotDiscoveryFileCrawls !== 13 || status.evidenceLevels?.[1]?.evidence?.verifiedDistinctContentPaths !== 30 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 8 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotContentCrawls !== 0 || status.evidenceLevels?.[1]?.evidence?.historicallyVerifiedBingbotContentCrawls !== 7) failures.push('/data/ai-search-evidence-status.json: crawler evidence is incomplete');
  if (status.evidenceLevels?.[1]?.evidence?.fixedPromptCoverage?.promptsWithAnyVerifiedCrawl !== 10 || status.evidenceLevels?.[1]?.evidence?.fixedPromptCoverage?.promptsFullyVerifiedCrawled !== 6 || status.evidenceLevels?.[1]?.evidence?.fixedPromptCoverage?.promptsWithAnySearchRelatedCrawl !== 0 || status.evidenceLevels?.[1]?.evidence?.fixedPromptCoverage?.scenarioPromptsWithAnyVerifiedCrawl !== 0) failures.push('/data/ai-search-evidence-status.json: fixed-prompt crawl coverage is incomplete');
