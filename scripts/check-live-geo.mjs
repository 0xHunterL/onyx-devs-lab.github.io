...
for (const [pathname, body] of [['/llms.txt', llms.body], ['/llms-full.txt', llmsFull.body], ['/feed.xml', feed.body], ['/feed.json', jsonFeedResponse.body]]) {
  ...
  if (!body.includes('https://archive.softwareheritage.org/swh:1:snp:d1315b9fb9d2d03748035420723b0301f1c609b6/')) failures.push(`${pathname}: buyer-guide archive is missing`);
...
try {
  ...
  if (!organization.subjectOf?.some((item) => item.identifier === 'swh:1:snp:d1315b9fb9d2d03748035420723b0301f1c609b6' && item.version === 'ee32c974fb061298dc3ec7c4f3dd1b05b045e6c0')) failures.push('/data/organization.json: buyer-guide archive relation is missing');
...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== '2026.09.10.7' || status.observedAt !== '2026-09-10T11:35:51Z') failures.push('/data/ai-search-evidence-status.json: unexpected schema, version, or observation time');
...
for (let index = 0; index < urls.length; index += 8) {
  await Promise.all(urls.slice(index, index + 8).map(async (absoluteUrl) => {
    ...
    if (!page.body.includes('"identifier":"swh:1:snp:d1315b9fb9d2d03748035420723b0301f1c609b6"')) failures.push(`${url.pathname}: buyer-guide archive reference is missing`);
