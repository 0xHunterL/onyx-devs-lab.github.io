...
try {
  ...
  if (status.evidenceLevels?.[3]?.evidence?.trackedAttributionRequests !== 73 || status.evidenceLevels?.[3]?.evidence?.suspectedAutomatedTrackedRequests !== 61 || status.evidenceLevels?.[3]?.evidence?.humanUnverifiedTrackedRequests !== 12 || status.evidenceLevels?.[3]?.evidence?.knownLinkScannerTrackedRequests !== 14 || status.evidenceLevels?.[3]?.evidence?.knownLinkScannerNetworkRequests !== 2 || status.evidenceLevels?.[3]?.evidence?.internallyInconsistentUserAgentRequests !== 4 || status.evidenceLevels?.[3]?.evidence?.malformedCampaignRequests !== 6 || status.evidenceLevels?.[3]?.evidence?.realAiReferralVisitsObserved !== 0 || status.evidenceLevels?.[3]?.evidence?.latestVerifiedOffsiteReferral?.referrerHost !== 'mixuechu.github.io' || status.evidenceLevels?.[3]?.evidence?.latestVisitorTypeUnverifiedAttributions?.length !== 3) failures.push('/data/ai-search-evidence-status.json: referral evidence boundary is incomplete');
} catch {
