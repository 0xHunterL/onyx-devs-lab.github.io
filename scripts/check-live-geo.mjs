...
try {
  ...
  if (status.evidenceLevels?.[1]?.evidence?.userAgentOnlyBytespiderContentCrawlCandidates !== currentUnverifiedCrawlerEvidence.bytespiderContentRequestCandidates || status.evidenceLevels?.[1]?.evidence?.userAgentOnlyBytespiderDiscoveryFileCandidates !== currentUnverifiedCrawlerEvidence.bytespiderDiscoveryFileRequestCandidates || status.evidenceLevels?.[1]?.evidence?.syntheticBytespiderReleaseChecksExcluded !== currentUnverifiedCrawlerEvidence.syntheticBytespiderReleaseChecksExcluded || !status.evidenceLevels?.[1]?.limitations?.some((item) => item.includes('do not prove Doubao or ByteDance access'))) failures.push('/data/ai-search-evidence-status.json: identity-unverified Bytespider boundary is incomplete');
