import {
  ...
  currentDomainCanonicalizationEvidence,
  currentMonitorEvidence,
  currentPromptCoverage,
  currentProviderEvidence,
  currentPublicSearchRetest,
  currentGooglePublicSearchAttempt,
  currentYandexPublicSearchEvidence,
...
try {
  ...
  if (!jsonFeed.items.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-prompt-crawl-coverage-2026-09-10' && item.date_modified === '2026-09-10T00:00:00+08:00')) failures.push('/feed.json: fixed-prompt crawl coverage checkpoint is missing or stale');
  if (!jsonFeed.items.some((item) => item.tags?.includes('independent-archive') && item.url === 'https://archive.softwareheritage.org/swh:1:snp:beec7f09ab04a666d11d120abddf542620d09e97/')) failures.push('/feed.json: Software Heritage archive item is missing');
  ...
  if (!feed.body.includes('<id>https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/resources.json</id><link href="https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/resources.json"/><updated>2026-09-11T00:00:00+08:00</updated>')) failures.push('/feed.xml: buyer-guide resource-map entry is missing or stale');
...
for (const [pathname, body] of [['/llms.txt', llms.body], ['/llms-full.txt', llmsFull.body], ['/feed.xml', feed.body], ['/feed.json', jsonFeedResponse.body]]) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-prompt-crawl-coverage-2026-09-10')) failures.push(`${pathname}: fixed-prompt crawl coverage checkpoint is missing`);
  ...
  if (!body.includes('https://archive.softwareheritage.org/swh:1:snp:beec7f09ab04a666d11d120abddf542620d09e97/')) failures.push(`${pathname}: Software Heritage snapshot is missing`);
  ...
  for (const asset of ['CITATION.cff', 'codemeta.json', 'resources.json']) if (!body.includes(`https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/${asset}`)) failures.push(`${pathname}: buyer-guide metadata is missing: ${asset}`);
  if (!body.includes('https://github.com/mixuechu/hong-kong-enterprise-ai-buyers-guide/releases/tag/buyers-guide-machine-resources-2026-09-11')) failures.push(`${pathname}: versioned buyer-guide checkpoint is missing`);
  if (!body.includes('https://archive.softwareheritage.org/swh:1:snp:7258af88334a1d2c00a25ab0bbb9330b4936d863/')) failures.push(`${pathname}: buyer-guide archive is missing`);
...
try {
  ...
  if (organization.contactPoint?.['@type'] !== 'ContactPoint' || organization.contactPoint?.contactType !== 'sales' || organization.contactPoint?.email !== 'info@onyxdevslab.com' || organization.contactPoint?.availableLanguage?.join(',') !== 'English,Chinese') failures.push('/data/organization.json: canonical sales contact relation is incomplete');
  ...
  if (!organization.subjectOf?.some((item) => item.url === 'https://www.tempb.com/companies/onyx-devs-lab-limited/' && item.description?.includes('entity-disambiguation evidence only'))) failures.push('/data/organization.json: tempb entity-disambiguation record or boundary is missing');
  if (!organization.subjectOf?.some((item) => item.url === 'https://hkg.databasesets.com/zh-hant/gongsimingdan/number/79051925' && item.description?.includes('entity-disambiguation evidence only'))) failures.push('/data/organization.json: databasesets entity-disambiguation record or boundary is missing');
  if (!organization.subjectOf?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-15-78' && item.sameAs === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-15-78/2026-09-11-monitor-evidence.json' && item.distribution?.contentUrl === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-15-78/ai-search-evidence-status.json')) failures.push('/data/organization.json: automated GEO monitor evidence relation is missing');
  if (!organization.subjectOf?.some((item) => item.identifier === 'swh:1:snp:beec7f09ab04a666d11d120abddf542620d09e97' && item.version === 'd7d6ee308e5f543e3136d3da68c2fd16f0361500')) failures.push('/data/organization.json: Software Heritage snapshot relation is missing');
  ...
  if (organization.member?.length !== 5 || !organization.member.every((person) => person['@type'] === 'Person' && person['@id']?.startsWith('https://hk.onyxdevslab.com/#person-') && person.name && person.jobTitle && person.url === `https://hk.onyxdevslab.com/en/about/#${person.name.toLowerCase()}` && person.worksFor?.['@id'] === 'https://hk.onyxdevslab.com/#organization')) failures.push('/data/organization.json: canonical team members or resolvable profile URLs are incomplete');
} catch {
...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== currentMonitorEvidence.publicStatusVersion || status.observedAt !== currentMonitorEvidence.generatedAt || status.latestEvidenceObservedAt !== currentMonitorEvidence.latestEvidenceObservedAt) failures.push('/data/ai-search-evidence-status.json: unexpected schema, version, or observation time');
  if (status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== currentProviderEvidence.oaiSearchBotDiscoveryFileRequests) failures.push('/data/ai-search-evidence-status.json: current OAI-SearchBot discovery count is stale');
  if (JSON.stringify(status.providerVerifiedCrawlerEvidence) !== JSON.stringify(currentProviderEvidence) || !['googlebotDiscoveryFileRequests','perplexityDiscoveryFileRequests','commonCrawlBotContentRequests','commonCrawlBotDiscoveryFileRequests'].every((key) => Object.hasOwn(status.providerVerifiedCrawlerEvidence || {}, key))) failures.push('/data/ai-search-evidence-status.json: complete provider-verified crawler counters are missing or stale');
  if (JSON.stringify(status.crawlerEvidenceAccounting) !== JSON.stringify(currentMonitorEvidence.crawlerEvidenceAccounting) || status.crawlerEvidenceAccounting?.mode !== 'append-only-provider-verified-fingerprint-cumulative') failures.push('/data/ai-search-evidence-status.json: verified crawler accounting method is missing or stale');
  if ('sameAs' in status || status.versionHistory?.length !== 4 || status.versionHistory?.[0]?.version !== '2026.09.15.78' || status.versionHistory?.[0]?.relation !== 'current-immutable-checkpoint' || status.versionHistory?.[0]?.url !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-15-78/ai-search-evidence-status.json' || status.versionHistory?.[1]?.version !== '2026.09.14.77' || status.versionHistory?.[1]?.relation !== 'previous-immutable-checkpoint' || status.versionHistory?.[2]?.version !== '2026.09.14.76' || status.versionHistory?.[2]?.relation !== 'earlier-immutable-checkpoint' || status.versionHistory?.[3]?.version !== '2026.09.13.75' || status.versionHistory?.[3]?.relation !== 'earlier-immutable-checkpoint' || !status.sourceBaseline?.endsWith('/docs/geo-baselines/2026-09-11-monitor-evidence.json')) failures.push('/data/ai-search-evidence-status.json: current source and immutable version history are misleading or incomplete');
  if (!status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-15-78') || !status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-15-78/ai-search-evidence-status.json')) failures.push('/data/ai-search-evidence-status.json: latest immutable monitor checkpoint is not discoverable');
  if (JSON.stringify(status.publicCorpusMonitoring?.softwareHeritage) !== JSON.stringify(currentMonitorEvidence.softwareHeritageEvidence) || status.publicCorpusMonitoring?.softwareHeritage?.archiveCoverageStatus !== 'lagging') failures.push('/data/ai-search-evidence-status.json: Software Heritage coverage observation is missing or stale');
  if (JSON.stringify(status.domainCanonicalization) !== JSON.stringify(currentDomainCanonicalizationEvidence) || status.domainCanonicalization?.status !== 'noncompliant' || status.domainCanonicalization?.compliantTargets !== 2 || status.domainCanonicalization?.mitigatedTargets !== 4 || status.domainCanonicalization?.noncompliantTargets !== 4 || status.domainCanonicalization?.targets?.filter((item) => item.status === 'mitigated-noncompliant').length !== 4 || status.domainCanonicalization?.targets?.find((item) => item.id === 'https-www')?.reasons?.includes('https-downgrade-observed') !== true || status.domainCanonicalization?.githubPagesSetting?.httpsEnforced !== false || status.domainCanonicalization?.authorityObservation?.nameServers?.join(',') !== 'piper.ns.cloudflare.com,trey.ns.cloudflare.com' || status.domainCanonicalization?.authorityObservation?.cloudflareNameservers !== true || status.domainCanonicalization?.edgeObservation?.targetsChecked !== 6 || status.domainCanonicalization?.edgeObservation?.cloudflareSignaledTargets !== 6 || status.domainCanonicalization?.edgeObservation?.allTargetsCloudflareSignaled !== true) failures.push('/data/ai-search-evidence-status.json: domain canonicalization defect or evidence boundary is incomplete');
  if (status.evidenceLevels?.[2]?.evidence?.yandexPublicSearchObservation?.officialBrandResult?.url !== 'https://onyxdevslab.com/' || status.evidenceLevels?.[2]?.evidence?.yandexPublicSearchObservation?.officialBrandResult?.canonicalTarget !== 'https://hk.onyxdevslab.com/' || status.evidenceLevels?.[2]?.evidence?.yandexPublicSearchObservation?.followUpQueries?.[0]?.status !== 'unavailable' || status.publicSearchMonitoring?.yandex?.captchaCompleted !== false) failures.push('/data/ai-search-evidence-status.json: Yandex brand-result evidence or captcha boundary is missing');
  if (status.evidenceLevels?.[1]?.evidence?.verifiedBaiduspiderContentCrawls !== currentProviderEvidence.baiduspiderContentRequests || status.evidenceLevels?.[1]?.evidence?.verifiedBaiduspiderDiscoveryFileCrawls !== currentProviderEvidence.baiduspiderDiscoveryFileRequests || status.evidenceLevels?.[1]?.evidence?.verifiedApplebotContentCrawls !== currentProviderEvidence.applebotContentRequests || status.evidenceLevels?.[1]?.evidence?.verifiedApplebotDiscoveryFileCrawls !== currentProviderEvidence.applebotDiscoveryFileRequests || status.evidenceLevels?.[1]?.evidence?.verifiedYandexbotContentCrawls !== currentProviderEvidence.yandexbotContentRequests || status.evidenceLevels?.[1]?.evidence?.verifiedYandexbotDiscoveryFileCrawls !== currentProviderEvidence.yandexbotDiscoveryFileRequests || status.evidenceLevels?.[1]?.evidence?.verifiedAhrefsBotContentCrawls !== currentProviderEvidence.ahrefsbotContentRequests || status.evidenceLevels?.[1]?.evidence?.verifiedAhrefsBotDiscoveryFileCrawls !== currentProviderEvidence.ahrefsbotDiscoveryFileRequests) failures.push('/data/ai-search-evidence-status.json: Baiduspider, Applebot, YandexBot, or AhrefsBot verified baseline is missing');
  if (status.publicSearchMonitoring?.bing?.status !== 'partial-public-ui-observation' || status.publicSearchMonitoring?.bing?.verifiedSiteMetricsStatus !== 'account-verification-required' || status.publicSearchMonitoring?.bing?.publicWebObservation?.queries?.[0]?.result !== 'official-site-observed-in-first-visible-page' || status.publicSearchMonitoring?.bing?.publicWebObservation?.previousObservation?.exactMarkerResult !== 'explicit-no-results' || !status.publicSearchMonitoring?.bing?.restrictedPathExcluded?.includes('personal, non-commercial') || !status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-search-monitoring-compliance-2026-09-11')) failures.push('/data/ai-search-evidence-status.json: compliant Bing monitoring boundary is incomplete');
  if (JSON.stringify(status.evidenceLevels?.[2]?.evidence?.bingPublicSearchObservation) !== JSON.stringify(currentBingPublicSearchEvidence) || JSON.stringify(status.evidenceLevels?.[2]?.evidence?.googlePublicSearchAttempt) !== JSON.stringify(currentGooglePublicSearchAttempt) || currentBingPublicSearchEvidence.queries?.length !== 5 || currentGooglePublicSearchAttempt.captchaCompleted !== false || currentGooglePublicSearchAttempt.resultConclusionDrawn !== false) failures.push('/data/ai-search-evidence-status.json: provider-attributed Bing/Google search evidence or captcha boundary is incomplete');
  if (status.publicSearchMonitoring?.queryGrouping?.neutralCategory !== '"Hong Kong AI consulting"' || status.publicSearchMonitoring?.queryGrouping?.fdeCategory !== '香港有哪些提供 FDE 前线部署工程的团队？' || !status.publicSearchMonitoring?.queryGrouping?.reason?.includes('confound')) failures.push('/data/ai-search-evidence-status.json: neutral and FDE category query groups are not separated');
  if (JSON.stringify(status.offsiteChannelReadiness?.servicesHk) !== JSON.stringify(currentMonitorEvidence.servicesHkReadinessEvidence) || currentMonitorEvidence.servicesHkReadinessEvidence?.targetsChecked !== 3 || currentMonitorEvidence.servicesHkReadinessEvidence?.availableTargets !== 0) failures.push('/data/ai-search-evidence-status.json: services.hk readiness baseline is missing or stale');
  if (status.testProtocol?.schemaVersion !== 3 || status.testProtocol?.promptCount !== 23 || status.testProtocol?.queryAliasesAdded?.join(',') !== 'AI 定开,AI定开' || !status.testProtocol?.protocolChange?.includes('all six published cases')) failures.push('/data/ai-search-evidence-status.json: current prompt protocol or query-alias coverage is incomplete');
  ...
  if (JSON.stringify(status.evidenceLevels?.[2]?.evidence?.publicSearchObservation) !== JSON.stringify(currentPublicSearchRetest) || currentPublicSearchRetest.brandSearchResultsObserved?.length !== 1 || currentPublicSearchRetest.previousBrandSearchObservation?.resultCount !== 5 || currentPublicSearchRetest.lowConfidenceEntityDirectoryResults?.length !== 0 || currentPublicSearchRetest.historicalLowConfidenceEntityDirectoryResults?.length !== 1 || !currentPublicSearchRetest.historicalLowConfidenceEntityDirectoryResults[0].treatment.includes('not accepted as an authoritative entity source')) failures.push('/data/ai-search-evidence-status.json: public-search result quality or history boundary is missing or stale');
  if (currentPublicSearchRetest.nonBrandCategoryResultsObserved?.length !== 22 || !currentPublicSearchRetest.nonBrandCategoryResultsObserved.some((item) => item.url === 'https://www.alphamatch.ai/solutions/ai-consulting') || !currentPublicSearchRetest.nonBrandCategoryResultsObserved.some((item) => item.url === 'https://aikm.ai/zh-hans/consulting') || !currentPublicSearchRetest.nonBrandCategoryResultsObserved.some((item) => item.url === 'https://www.investhk.gov.hk/media/wvke34zz/202407-client-profiles-sc.pdf') || currentPublicSearchRetest.previousNonBrandCategoryObservation?.resultCount !== 23 || !currentPublicSearchRetest.nonBrandCategoryMeaning?.includes('not a complete ranking') || !currentPublicSearchRetest.nonBrandCategoryMeaning?.includes('does not establish a ranking trend')) failures.push('/data/ai-search-evidence-status.json: non-brand category result baseline is missing or misleading');
  ...
  if (status.publicSearchChecks?.length !== 10 || status.publicSearchChecks[0]?.checkedAt !== currentYandexPublicSearchEvidence.testedAt || status.publicSearchChecks[0]?.provider !== 'Yandex Web Search' || !status.publicSearchChecks[0]?.result.includes('official-apex-domain-observed') || !status.publicSearchChecks.slice(1,6).every((item) => item.checkedAt === currentBingPublicSearchEvidence.testedAt && item.provider === 'Bing Web Search') || status.publicSearchChecks[1]?.result !== 'official-site-observed-in-first-visible-page' || !status.publicSearchChecks.some((item) => item.query === '"Hong Kong AI consulting"' && item.result.includes('official-Onyx-result-not-observed')) || !status.publicSearchChecks.slice(6).every((item) => item.checkedAt === currentPublicSearchRetest.testedAt && item.result.includes('observed')) || !status.publicSearchChecks.some((item) => item.query === '香港 企业 AI 咨询 公司 FDE' && item.result.includes('official-Onyx-result-not-observed'))) failures.push('/data/ai-search-evidence-status.json: dated public-search checks are incomplete');
  if (JSON.stringify(status.platformSearchChecks?.githubRepository) !== JSON.stringify(currentMonitorEvidence.githubRepositorySearchEvidence) || status.platformSearchChecks?.githubRepository?.status !== 'available' || status.platformSearchChecks?.githubRepository?.queries?.find((item) => item.id === 'category')?.firstPartyRepositoriesObserved?.length !== 2 || !status.evidenceSources?.some((item) => item.url === 'https://hk.onyxdevslab.com/data/github-repository-search-baseline.json')) failures.push('/data/ai-search-evidence-status.json: GitHub platform-search evidence or source is incomplete');
...
try {
  ...
  if (promptMap.promptMatrix?.doubaoPromptsSent !== false || promptMap.totals?.verifiedCrawledEvidencePages !== 21 || promptMap.totals?.searchRelatedCrawledEvidencePages !== 16) failures.push('/data/ai-search-prompt-evidence-map.json: evidence boundary is incomplete');
...
if (!feed.body.includes('<updated>2026-09-15T00:00:00+08:00</updated>')) failures.push('/feed.xml: feed update date is stale');
if (!feed.body.includes('<id>https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-15-78</id><link href="https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-15-78"/><updated>2026-09-15T00:00:00+08:00</updated>')) failures.push('/feed.xml: automated monitor evidence checkpoint entry is missing or stale');
if (!jsonFeed?.items?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-15-78' && item.date_modified === '2026-09-15T00:00:00+08:00')) failures.push('/feed.json: automated monitor evidence checkpoint entry is missing or stale');
...
for (const pathname of ['/en/about/', '/zh-hk/about/', '/zh-cn/about/']) {
  ...
  for (const person of ['mi', 'lucas', 'hunter', 'jake', 'olivia']) {
    if (!page.body.includes(`"@type":"Person","@id":"https://hk.onyxdevslab.com/#person-${person}"`)) failures.push(`${pathname}: canonical Person node is missing: ${person}`);
    if (!page.body.includes(`<article class="person" id="${person}">`)) failures.push(`${pathname}: visible Person anchor is missing: ${person}`);
  }
}
...
for (let index = 0; index < urls.length; index += 8) {
  await Promise.all(urls.slice(index, index + 8).map(async (absoluteUrl) => {
    ...
    if (!page.body.includes('"identifier":"swh:1:snp:beec7f09ab04a666d11d120abddf542620d09e97"')) failures.push(`${url.pathname}: Software Heritage snapshot reference is missing`);
    if (!page.body.includes('"identifier":"swh:1:snp:7258af88334a1d2c00a25ab0bbb9330b4936d863"')) failures.push(`${url.pathname}: buyer-guide archive reference is missing`);
