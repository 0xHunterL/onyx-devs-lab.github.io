...
if (!llms.body.includes('2026-09-12T19:33:18.402Z baseline') || !llms.body.includes('provider-maintained checkpoint proves crawler and public archive-query state only, not indexing')) failures.push('/llms.txt: current monitor checkpoint lacks its observation-time or evidence boundary');
...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== currentMonitorEvidence.publicStatusVersion || status.observedAt !== currentMonitorEvidence.generatedAt) failures.push('/data/ai-search-evidence-status.json: unexpected schema, version, or observation time');
  ...
  if ('sameAs' in status || status.versionHistory?.length !== 4 || status.versionHistory?.[0]?.version !== '2026.09.12.51' || status.versionHistory?.[0]?.relation !== 'current-immutable-checkpoint' || status.versionHistory?.[0]?.url !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-12-51/ai-search-evidence-status.json' || status.versionHistory?.[1]?.version !== '2026.09.12.50' || status.versionHistory?.[1]?.relation !== 'previous-immutable-checkpoint' || status.versionHistory?.[2]?.version !== '2026.09.12.49' || status.versionHistory?.[2]?.relation !== 'earlier-immutable-checkpoint' || status.versionHistory?.[3]?.version !== '2026.09.12.41' || status.versionHistory?.[3]?.relation !== 'earlier-immutable-checkpoint' || !status.sourceBaseline?.endsWith('/docs/geo-baselines/2026-09-11-monitor-evidence.json')) failures.push('/data/ai-search-evidence-status.json: current source and immutable version history are misleading or incomplete');
  if (!status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-51') || !status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-12-51/ai-search-evidence-status.json')) failures.push('/data/ai-search-evidence-status.json: latest immutable monitor checkpoint is not discoverable');
...
if (!feed.body.includes('<id>https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-51</id><link href="https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-51"/><updated>2026-09-12T00:00:00+08:00</updated>')) failures.push('/feed.xml: automated monitor evidence checkpoint entry is missing or stale');
if (!jsonFeed?.items?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-51' && item.date_modified === '2026-09-12T00:00:00+08:00')) failures.push('/feed.json: automated monitor evidence checkpoint entry is missing or stale');
