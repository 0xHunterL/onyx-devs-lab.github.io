...
for (const asset of ['2026-09-11-monitor-evidence.json', 'ai-search-evidence-status.json']) if (!llms.body.includes(`https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-12-41/${asset}`)) failures.push(`/llms.txt: current monitor evidence asset is missing: ${asset}`);
...
if (!llms.body.includes('2026-09-12T10:04:54.078Z baseline') || !llms.body.includes('provider-maintained checkpoint does not prove indexing')) failures.push('/llms.txt: current monitor checkpoint lacks its observation-time or evidence boundary');
...
try {
  ...
  if (!jsonFeed.items.some((item) => item.tags?.includes('independent-archive') && item.url === 'https://archive.softwareheritage.org/swh:1:snp:df2409f12f9b01e665ae896d0492aa09148b9c1b/')) failures.push('/feed.json: Software Heritage archive item is missing');
...
for (const [pathname, body] of [['/llms.txt', llms.body], ['/llms-full.txt', llmsFull.body], ['/feed.xml', feed.body], ['/feed.json', jsonFeedResponse.body]]) {
  ...
  if (!body.includes('https://archive.softwareheritage.org/swh:1:snp:df2409f12f9b01e665ae896d0492aa09148b9c1b/')) failures.push(`${pathname}: Software Heritage snapshot is missing`);
...
try {
  ...
  if ('sameAs' in status || status.versionHistory?.length !== 2 || status.versionHistory?.[0]?.version !== '2026.09.12.41' || status.versionHistory?.[0]?.relation !== 'previous-immutable-checkpoint' || status.versionHistory?.[0]?.url !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-12-41/ai-search-evidence-status.json' || status.versionHistory?.[1]?.version !== '2026.09.11.8' || status.versionHistory?.[1]?.relation !== 'earlier-immutable-checkpoint' || status.versionHistory?.[1]?.url !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-search-monitoring-compliance-2026-09-11/ai-search-evidence-status.json' || !status.sourceBaseline?.endsWith('/docs/geo-baselines/2026-09-11-monitor-evidence.json')) failures.push('/data/ai-search-evidence-status.json: current source and immutable version history are misleading or incomplete');
  if (!status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-41') || !status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-12-41/ai-search-evidence-status.json')) failures.push('/data/ai-search-evidence-status.json: latest immutable monitor checkpoint is not discoverable');
...
if (!feed.body.includes('<id>https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-41</id><link href="https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-41"/><updated>2026-09-12T00:00:00+08:00</updated>')) failures.push('/feed.xml: automated monitor evidence checkpoint entry is missing or stale');
if (!jsonFeed?.items?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-41' && item.date_modified === '2026-09-12T00:00:00+08:00')) failures.push('/feed.json: automated monitor evidence checkpoint entry is missing or stale');
