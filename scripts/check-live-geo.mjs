...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== '2026.09.11.1' || status.observedAt !== '2026-09-11T05:53:47Z') failures.push('/data/ai-search-evidence-status.json: unexpected schema, version, or observation time');
  if (status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 6) failures.push('/data/ai-search-evidence-status.json: current OAI-SearchBot discovery count is stale');
  if (status.sameAs !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-prompt-crawl-coverage-2026-09-10/ai-search-evidence-status.json') failures.push('/data/ai-search-evidence-status.json: current versioned release asset is missing');
  ...
  if (status.evidenceLevels?.[1]?.evidence?.verifiedGptBotContentCrawls !== 24 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 6 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotContentCrawls !== 0 || status.evidenceLevels?.[1]?.evidence?.historicallyVerifiedBingbotContentCrawls !== 7) failures.push('/data/ai-search-evidence-status.json: crawler evidence is incomplete');
