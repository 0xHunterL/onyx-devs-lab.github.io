...
try {
  ...
  if (currentPublicSearchRetest.nonBrandCategoryResultsObserved?.length !== 12 || !currentPublicSearchRetest.nonBrandCategoryResultsObserved.some((item) => item.url === 'https://www.fde.hk/zh-cn/products/enterprise-ai-diagnosis') || currentPublicSearchRetest.previousNonBrandCategoryObservation?.resultCount !== 11 || !currentPublicSearchRetest.nonBrandCategoryMeaning?.includes('not a complete ranking') || !currentPublicSearchRetest.nonBrandCategoryMeaning?.includes('does not establish a ranking trend')) failures.push('/data/ai-search-evidence-status.json: non-brand category result baseline is missing or misleading');
