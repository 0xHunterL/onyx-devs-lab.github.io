...
try {
  ...
  if ('sameAs' in status || status.versionHistory?.length !== 4 || status.versionHistory?.[0]?.version !== '2026.09.13.60' || status.versionHistory?.[0]?.relation !== 'current-immutable-checkpoint' || status.versionHistory?.[0]?.url !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-13-60/ai-search-evidence-status.json' || status.versionHistory?.[1]?.version !== '2026.09.13.59' || status.versionHistory?.[1]?.relation !== 'previous-immutable-checkpoint' || status.versionHistory?.[2]?.version !== '2026.09.13.58' || status.versionHistory?.[2]?.relation !== 'earlier-immutable-checkpoint' || status.versionHistory?.[3]?.version !== '2026.09.13.57' || status.versionHistory?.[3]?.relation !== 'earlier-immutable-checkpoint' || !status.sourceBaseline?.endsWith('/docs/geo-baselines/2026-09-11-monitor-evidence.json')) failures.push('/data/ai-search-evidence-status.json: current source and immutable version history are misleading or incomplete');
  if (!status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-60') || !status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-13-60/ai-search-evidence-status.json')) failures.push('/data/ai-search-evidence-status.json: latest immutable monitor checkpoint is not discoverable');
  if (JSON.stringify(status.domainCanonicalization) !== JSON.stringify(currentDomainCanonicalizationEvidence) || status.domainCanonicalization?.status !== 'noncompliant' || status.domainCanonicalization?.compliantTargets !== 3 || status.domainCanonicalization?.targets?.find((item) => item.id === 'https-www')?.reasons?.includes('https-downgrade-observed') !== true || status.domainCanonicalization?.githubPagesSetting?.httpsEnforced !== false) failures.push('/data/ai-search-evidence-status.json: domain canonicalization defect or evidence boundary is incomplete');
...
if (!feed.body.includes('<id>https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-60</id><link href="https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-60"/><updated>2026-09-13T00:00:00+08:00</updated>')) failures.push('/feed.xml: automated monitor evidence checkpoint entry is missing or stale');
if (!jsonFeed?.items?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-60' && item.date_modified === '2026-09-13T00:00:00+08:00')) failures.push('/feed.json: automated monitor evidence checkpoint entry is missing or stale');
