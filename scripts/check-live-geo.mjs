...
try {
  ...
  if (!jsonFeed.items.some((item) => item.tags?.includes('independent-archive') && item.url === 'https://archive.softwareheritage.org/swh:1:snp:f3820205ce07ab9df33d2f0db735bc4b25ca0347/')) failures.push('/feed.json: Software Heritage archive item is missing');
...
for (const [pathname, body] of [['/llms.txt', llms.body], ['/llms-full.txt', llmsFull.body], ['/feed.xml', feed.body], ['/feed.json', jsonFeedResponse.body]]) {
  ...
  if (!body.includes('https://archive.softwareheritage.org/swh:1:snp:f3820205ce07ab9df33d2f0db735bc4b25ca0347/')) failures.push(`${pathname}: Software Heritage snapshot is missing`);
...
try {
  ...
  if (!organization.subjectOf?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-69' && item.sameAs === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-13-69/2026-09-11-monitor-evidence.json' && item.distribution?.contentUrl === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-13-69/ai-search-evidence-status.json')) failures.push('/data/organization.json: automated GEO monitor evidence relation is missing');
  if (!organization.subjectOf?.some((item) => item.identifier === 'swh:1:snp:f3820205ce07ab9df33d2f0db735bc4b25ca0347' && item.version === '40d2c1d369ec835c5012256a6f2f84964e01305b')) failures.push('/data/organization.json: Software Heritage snapshot relation is missing');
...
try {
  ...
  if ('sameAs' in status || status.versionHistory?.length !== 4 || status.versionHistory?.[0]?.version !== '2026.09.13.69' || status.versionHistory?.[0]?.relation !== 'current-immutable-checkpoint' || status.versionHistory?.[0]?.url !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-13-69/ai-search-evidence-status.json' || status.versionHistory?.[1]?.version !== '2026.09.13.68' || status.versionHistory?.[1]?.relation !== 'previous-immutable-checkpoint' || status.versionHistory?.[2]?.version !== '2026.09.13.67' || status.versionHistory?.[2]?.relation !== 'earlier-immutable-checkpoint' || status.versionHistory?.[3]?.version !== '2026.09.13.66' || status.versionHistory?.[3]?.relation !== 'earlier-immutable-checkpoint' || !status.sourceBaseline?.endsWith('/docs/geo-baselines/2026-09-11-monitor-evidence.json')) failures.push('/data/ai-search-evidence-status.json: current source and immutable version history are misleading or incomplete');
  if (!status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-69') || !status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-13-69/ai-search-evidence-status.json')) failures.push('/data/ai-search-evidence-status.json: latest immutable monitor checkpoint is not discoverable');
  if (JSON.stringify(status.publicCorpusMonitoring?.softwareHeritage) !== JSON.stringify(currentMonitorEvidence.softwareHeritageEvidence) || status.publicCorpusMonitoring?.softwareHeritage?.archiveCoverageStatus !== 'current') failures.push('/data/ai-search-evidence-status.json: Software Heritage coverage observation is missing or stale');
...
if (!feed.body.includes('<id>https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-69</id><link href="https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-69"/><updated>2026-09-13T00:00:00+08:00</updated>')) failures.push('/feed.xml: automated monitor evidence checkpoint entry is missing or stale');
if (!jsonFeed?.items?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-69' && item.date_modified === '2026-09-13T00:00:00+08:00')) failures.push('/feed.json: automated monitor evidence checkpoint entry is missing or stale');
...
for (let index = 0; index < urls.length; index += 8) {
  await Promise.all(urls.slice(index, index + 8).map(async (absoluteUrl) => {
    ...
    if (!page.body.includes('"identifier":"swh:1:snp:f3820205ce07ab9df33d2f0db735bc4b25ca0347"')) failures.push(`${url.pathname}: Software Heritage snapshot reference is missing`);
