...
if (!crawlerReportSource.includes('userAgentOnlyBytespiderPageCrawls') || !crawlerReportSource.includes('user-agent-only-unverified') || !crawlerReportSource.includes('do not prove Doubao or ByteDance access')) failures.push('crawler report: identity-unverified Bytespider evidence boundary is missing');
...
for (const file of htmlFiles) {
  ...
  if (!html.includes('"identifier":"swh:1:snp:df2409f12f9b01e665ae896d0492aa09148b9c1b"')) failures.push(`${relative}: missing Software Heritage snapshot reference`);
...
for (const [name, body] of Object.entries(machineDiscoveryFiles)) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-11-6')) failures.push(`${name}: automated GEO monitor evidence checkpoint is missing`);
  ...
  if (!body.includes('https://archive.softwareheritage.org/swh:1:snp:df2409f12f9b01e665ae896d0492aa09148b9c1b/')) failures.push(`${name}: Software Heritage snapshot is missing`);
  ...
  if (!body.includes('https://github.com/mixuechu/hong-kong-enterprise-ai-buyers-guide/releases/tag/buyers-guide-scenarios-2026-09-11')) failures.push(`${name}: versioned buyer-guide checkpoint is missing`);
...
try {
  ...
  if (!jsonFeed.items.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-11-6' && item.date_modified === '2026-09-11T00:00:00+08:00')) failures.push('feed.json: automated monitor evidence checkpoint entry is missing or stale');
...
try {
  ...
  if (!organization.subjectOf?.some((item) => item.identifier === 'swh:1:snp:df2409f12f9b01e665ae896d0492aa09148b9c1b' && item.version === 'f17606cf302d4e1368eae3f2a76c295af61384ad')) failures.push('organization record: Software Heritage snapshot relation is missing');
  ...
  if (!organization.subjectOf?.some((item) => item.url === 'https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/' && item.sameAs === 'https://github.com/mixuechu/hong-kong-enterprise-ai-buyers-guide/releases/tag/buyers-guide-scenarios-2026-09-11' && item.hasPart?.length === 5 && item.hasPart.some((part) => part.url === 'https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/enterprise-ai-scenario-patterns/') && item.hasPart.every((part) => part.url?.startsWith('https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/')))) failures.push('organization record: focused buyer-guide relations are missing');
...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== '2026.09.11.6' || status.observedAt !== '2026-09-11T09:03:06Z') failures.push('AI-search evidence status: unexpected schema, version, or observation time');
  ...
  if (status.sameAs !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-11-6/2026-09-11-monitor-evidence.json') failures.push('AI-search evidence status: current versioned release asset is missing');
  ...
  if (status.evidenceLevels?.[1]?.evidence?.verifiedGptBotContentCrawls !== 24 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 8 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotContentCrawls !== 0 || status.evidenceLevels?.[1]?.evidence?.historicallyVerifiedBingbotContentCrawls !== 7) failures.push('AI-search evidence status: crawler evidence is incomplete');
  ...
  if (status.evidenceLevels?.[1]?.evidence?.userAgentOnlyBytespiderContentCrawlCandidates !== 0 || status.evidenceLevels?.[1]?.evidence?.userAgentOnlyBytespiderDiscoveryFileCandidates !== 0 || status.evidenceLevels?.[1]?.evidence?.syntheticBytespiderReleaseChecksExcluded !== 2368 || !status.evidenceLevels?.[1]?.limitations?.some((item) => item.includes('do not prove Doubao or ByteDance access'))) failures.push('AI-search evidence status: identity-unverified Bytespider boundary is incomplete');
  if (status.evidenceLevels?.[3]?.evidence?.trackedAttributionRequests !== 70 || status.evidenceLevels?.[3]?.evidence?.suspectedAutomatedTrackedRequests !== 61 || status.evidenceLevels?.[3]?.evidence?.humanUnverifiedTrackedRequests !== 9 || status.evidenceLevels?.[3]?.evidence?.knownLinkScannerTrackedRequests !== 14 || status.evidenceLevels?.[3]?.evidence?.knownLinkScannerNetworkRequests !== 2 || status.evidenceLevels?.[3]?.evidence?.internallyInconsistentUserAgentRequests !== 4 || status.evidenceLevels?.[3]?.evidence?.malformedCampaignRequests !== 6 || status.evidenceLevels?.[3]?.evidence?.realAiReferralVisitsObserved !== 0 || status.evidenceLevels?.[3]?.evidence?.latestVerifiedOffsiteReferral?.referrerHost !== 'mixuechu.github.io') failures.push('AI-search evidence status: referral evidence boundary is incomplete');
} catch {
