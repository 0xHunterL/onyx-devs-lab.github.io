...
for (const [name, body] of Object.entries(machineDiscoveryFiles)) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-11-5')) failures.push(`${name}: automated GEO monitor evidence checkpoint is missing`);
...
try {
  ...
  if (!jsonFeed.items.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-11-5' && item.date_modified === '2026-09-11T00:00:00+08:00')) failures.push('feed.json: automated monitor evidence checkpoint entry is missing or stale');
...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== '2026.09.11.5' || status.observedAt !== '2026-09-11T08:27:38Z') failures.push('AI-search evidence status: unexpected schema, version, or observation time');
  ...
  if (status.sameAs !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-11-5/2026-09-11-monitor-evidence.json') failures.push('AI-search evidence status: current versioned release asset is missing');
  ...
  if (status.evidenceLevels?.[3]?.evidence?.trackedAttributionRequests !== 69 || status.evidenceLevels?.[3]?.evidence?.suspectedAutomatedTrackedRequests !== 60 || status.evidenceLevels?.[3]?.evidence?.humanUnverifiedTrackedRequests !== 9 || status.evidenceLevels?.[3]?.evidence?.knownLinkScannerTrackedRequests !== 13 || status.evidenceLevels?.[3]?.evidence?.knownLinkScannerNetworkRequests !== 1 || status.evidenceLevels?.[3]?.evidence?.internallyInconsistentUserAgentRequests !== 4 || status.evidenceLevels?.[3]?.evidence?.malformedCampaignRequests !== 6 || status.evidenceLevels?.[3]?.evidence?.realAiReferralVisitsObserved !== 0 || status.evidenceLevels?.[3]?.evidence?.latestVerifiedOffsiteReferral?.referrerHost !== 'mixuechu.github.io') failures.push('AI-search evidence status: referral evidence boundary is incomplete');
} catch {
