...
for (const [name, body] of Object.entries(machineDiscoveryFiles)) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-11-7')) failures.push(`${name}: automated GEO monitor evidence checkpoint is missing`);
...
try {
  ...
  if (status.evidenceLevels?.[1]?.evidence?.verifiedGptBotContentCrawls !== 27 || status.evidenceLevels?.[1]?.evidence?.verifiedGptBotDiscoveryFileCrawls !== 13 || status.evidenceLevels?.[1]?.evidence?.verifiedDistinctContentPaths !== 30 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 8 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotContentCrawls !== 0 || status.evidenceLevels?.[1]?.evidence?.historicallyVerifiedBingbotContentCrawls !== 7) failures.push('AI-search evidence status: crawler evidence is incomplete');
