...
if (currentWaybackEvidence.fixedPromptArchiveCoverage?.prompts !== 20 || currentWaybackEvidence.fixedPromptArchiveCoverage?.uniqueEvidencePages !== 20 || currentWaybackEvidence.fixedPromptArchiveCoverage?.archivedEvidencePages !== 15 || currentWaybackEvidence.fixedPromptArchiveCoverage?.promptsWithAnyArchivedEvidence !== 15 || currentWaybackEvidence.fixedPromptArchiveCoverage?.promptsFullyArchived !== 12 || currentWaybackEvidence.fixedPromptArchiveCoverage?.missingEvidenceUrls?.length !== 5) failures.push('Wayback fixed-prompt archive coverage: published totals are incomplete or stale');
...
if (promptCrawlCoverage.totals?.prompts !== 20 || promptCrawlCoverage.totals?.uniqueEvidencePages !== 20 || promptCrawlCoverage.totals?.verifiedCrawledEvidencePages !== 12 || promptCrawlCoverage.totals?.searchRelatedCrawledEvidencePages !== 12 || promptCrawlCoverage.totals?.promptsWithAnyVerifiedCrawl !== 15 || promptCrawlCoverage.totals?.promptsFullyVerifiedCrawled !== 9 || promptCrawlCoverage.totals?.promptsWithAnySearchRelatedCrawl !== 15 || promptCrawlCoverage.totals?.promptsFullySearchRelatedCrawled !== 9) failures.push('prompt crawl coverage: verified coverage totals are incomplete or stale');
...
if (promptEvidenceUrls.size !== 20 || !promptCrawlCoverage.verifiedCrawledEvidenceUrls?.every((url) => promptEvidenceUrls.has(url)) || !promptCrawlCoverage.searchRelatedCrawledEvidenceUrls?.every((url) => promptEvidenceUrls.has(url))) failures.push('prompt crawl coverage: fixed prompt matrix is not fully represented');
...
if (promptCrawlCoverage.bySegment?.scenario?.promptsWithAnyVerifiedCrawl !== 1) failures.push('prompt crawl coverage: scenario-page result is stale');
...
for (const [name, body] of Object.entries(machineDiscoveryFiles)) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-41')) failures.push(`${name}: automated GEO monitor evidence checkpoint is missing`);
...
for (const asset of ['2026-09-11-monitor-evidence.json', 'ai-search-evidence-status.json']) if (!machineDiscoveryFiles['llms.txt'].includes(`https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-12-41/${asset}`)) failures.push(`llms.txt: current monitor evidence asset is missing: ${asset}`);
...
if (!machineDiscoveryFiles['llms.txt'].includes('2026-09-12T10:04:54.078Z baseline') || !machineDiscoveryFiles['llms.txt'].includes('provider-maintained checkpoint does not prove indexing')) failures.push('llms.txt: current monitor checkpoint lacks its observation-time or evidence boundary');
...
try {
  ...
  if (promptMap.promptMatrix?.doubaoPromptsSent !== false || promptMap.totals?.verifiedCrawledEvidencePages !== 12 || promptMap.totals?.searchRelatedCrawledEvidencePages !== 12) failures.push('AI-search prompt evidence map: evidence boundary is incomplete');
...
try {
  ...
  if (currentPublicSearchRetest.nonBrandCategoryResultsObserved?.length !== 12 || !currentPublicSearchRetest.nonBrandCategoryResultsObserved.some((item) => item.url === 'https://www.fde.hk/zh-cn/products/enterprise-ai-diagnosis') || currentPublicSearchRetest.previousNonBrandCategoryObservation?.resultCount !== 11 || !currentPublicSearchRetest.nonBrandCategoryMeaning?.includes('not a complete ranking') || !currentPublicSearchRetest.nonBrandCategoryMeaning?.includes('does not establish a ranking trend')) failures.push('AI-search evidence status: non-brand category result baseline is missing or misleading');
