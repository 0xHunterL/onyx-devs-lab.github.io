...
if (currentWaybackEvidence.fixedPromptArchiveCoverage?.prompts !== 23 || currentWaybackEvidence.fixedPromptArchiveCoverage?.uniqueEvidencePages !== 23 || currentWaybackEvidence.fixedPromptArchiveCoverage?.archivedEvidencePages !== 16 || currentWaybackEvidence.fixedPromptArchiveCoverage?.promptsWithAnyArchivedEvidence !== 16 || currentWaybackEvidence.fixedPromptArchiveCoverage?.promptsFullyArchived !== 12 || currentWaybackEvidence.fixedPromptArchiveCoverage?.missingEvidenceUrls?.length !== 7) failures.push('Wayback fixed-prompt archive coverage: published totals are incomplete or stale');
...
for (const [name, body] of Object.entries(machineDiscoveryFiles)) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-65')) failures.push(`${name}: automated GEO monitor evidence checkpoint is missing`);
