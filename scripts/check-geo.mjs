...
for (const [name, body] of Object.entries(machineDiscoveryFiles)) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-13-56')) failures.push(`${name}: automated GEO monitor evidence checkpoint is missing`);
...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== currentMonitorEvidence.publicStatusVersion || status.observedAt !== currentMonitorEvidence.generatedAt || status.latestEvidenceObservedAt !== currentYandexPublicSearchEvidence.testedAt) failures.push('AI-search evidence status: unexpected schema, version, or observation time');
