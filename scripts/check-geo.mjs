...
for (const file of htmlFiles) {
  ...
  if (!html.includes('"identifier":"swh:1:snp:d1315b9fb9d2d03748035420723b0301f1c609b6"')) failures.push(`${relative}: missing buyer-guide archive reference`);
...
for (const [name, body] of Object.entries(machineDiscoveryFiles)) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-11')) failures.push(`${name}: automated GEO monitor evidence checkpoint is missing`);
  ...
  if (!body.includes('https://archive.softwareheritage.org/swh:1:snp:d1315b9fb9d2d03748035420723b0301f1c609b6/')) failures.push(`${name}: buyer-guide archive is missing`);
...
try {
  ...
  if (!jsonFeed.items.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-11' && item.date_modified === '2026-09-11T00:00:00+08:00')) failures.push('feed.json: automated monitor evidence checkpoint entry is missing or stale');
  ...
  if (!jsonFeed.items.some((item) => item.tags?.includes('independent-archive') && item.url === 'https://archive.softwareheritage.org/swh:1:snp:d1315b9fb9d2d03748035420723b0301f1c609b6/')) failures.push('feed.json: buyer-guide archive item is missing');
  for (const [pathname, timestamp] of archivedAnswerPages) {
    const archiveUrl = `https://web.archive.org/web/${timestamp}/https://hk.onyxdevslab.com/zh-cn/${pathname}/`;
...
try {
  ...
  if (!organization.subjectOf?.some((item) => item.identifier === 'swh:1:snp:d1315b9fb9d2d03748035420723b0301f1c609b6' && item.version === 'ee32c974fb061298dc3ec7c4f3dd1b05b045e6c0')) failures.push('organization record: buyer-guide archive relation is missing');
...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== '2026.09.10.7' || status.observedAt !== '2026-09-10T11:35:51Z') failures.push('AI-search evidence status: unexpected schema, version, or observation time');
  if (status.sameAs !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-prompt-crawl-coverage-2026-09-10/ai-search-evidence-status.json') failures.push('AI-search evidence status: current versioned release asset is missing');
  ...
  if (status.evidenceLevels?.[1]?.evidence?.verifiedGptBotContentCrawls !== 24 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 5 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotContentCrawls !== 0 || status.evidenceLevels?.[1]?.evidence?.historicallyVerifiedBingbotContentCrawls !== 7) failures.push('AI-search evidence status: crawler evidence is incomplete');
