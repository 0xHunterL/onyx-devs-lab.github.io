...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== '2026.09.11.1' || status.observedAt !== '2026-09-11T05:53:47Z') failures.push('AI-search evidence status: unexpected schema, version, or observation time');
  if (status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 6) failures.push('AI-search evidence status: current OAI-SearchBot discovery count is stale');
  ...
  if (status.evidenceLevels?.[1]?.evidence?.verifiedGptBotContentCrawls !== 24 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 6 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotContentCrawls !== 0 || status.evidenceLevels?.[1]?.evidence?.historicallyVerifiedBingbotContentCrawls !== 7) failures.push('AI-search evidence status: crawler evidence is incomplete');
