...
for (const [name, body] of Object.entries(machineDiscoveryFiles)) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-51')) failures.push(`${name}: automated GEO monitor evidence checkpoint is missing`);
...
if (!machineDiscoveryFiles['llms.txt'].includes('2026-09-12T19:33:18.402Z baseline') || !machineDiscoveryFiles['llms.txt'].includes('provider-maintained checkpoint proves crawler and public archive-query state only, not indexing')) failures.push('llms.txt: current monitor checkpoint lacks its observation-time or evidence boundary');
