...
const softwareHeritageReportSource = fs.readFileSync(path.resolve('scripts/report-software-heritage.mjs'), 'utf8');
const distributionLiveSource = fs.readFileSync(path.resolve('scripts/check-distribution-live.mjs'), 'utf8');
const domainCanonicalizationSource = fs.readFileSync(path.resolve('scripts/check-domain-canonicalization.mjs'), 'utf8');
...
if (!referralReportSource.includes('Onyx-(?:GEO-[A-Za-z0-9-]+|Buyer-Guide-Link-Check)')) failures.push('referral report: the internal Onyx GEO user-agent namespace is not excluded as synthetic');
...
if (!crawlerReportSource.includes("--verify-yandex") || !crawlerReportSource.includes("['.yandex.ru', '.yandex.net', '.yandex.com']") || !crawlerReportSource.includes('official-reverse-dns-suffix-plus-forward-confirmation') || !evidenceCollectorSource.includes("'--verify-yandex'")) failures.push('crawler report: official YandexBot DNS verification is not enabled in the production collector');
...
if (!evidenceCollectorSource.includes("report-software-heritage.mjs', []") || !evidenceCollectorSource.includes("atomicJson('software-heritage-report.json'") || !softwareHeritageReportSource.includes('/visits/?limit=20') || !softwareHeritageReportSource.includes('/origin/save/${saveRequestId}/') || !softwareHeritageReportSource.includes('software-heritage-save-request') || !softwareHeritageReportSource.includes('software-heritage-snapshot') || !softwareHeritageReportSource.includes("visit?.status === 'full'")) failures.push('evidence monitor: current Software Heritage archive and save-request coverage is not collected or persisted');
...
if (!domainCanonicalizationSource.includes("resolveNsImpl(authorityDomain)") || !domainCanonicalizationSource.includes("edgeServer: response.headers.get('server')") || !domainCanonicalizationSource.includes("Boolean(response.headers.get('cf-ray'))") || !evidenceSummarySource.includes('authorityObservation: report?.authorityObservation') || !evidenceSummarySource.includes('edgeObservation: report?.edgeObservation')) failures.push('domain canonicalization monitor: authoritative DNS or public edge-provider evidence is missing');
...
if (currentWaybackEvidence.fixedPromptArchiveCoverage?.prompts !== 23 || currentWaybackEvidence.fixedPromptArchiveCoverage?.uniqueEvidencePages !== 23 || currentWaybackEvidence.fixedPromptArchiveCoverage?.archivedEvidencePages !== 18 || currentWaybackEvidence.fixedPromptArchiveCoverage?.promptsWithAnyArchivedEvidence !== 18 || currentWaybackEvidence.fixedPromptArchiveCoverage?.promptsFullyArchived !== 15 || currentWaybackEvidence.fixedPromptArchiveCoverage?.missingEvidenceUrls?.length !== 5) failures.push('Wayback fixed-prompt archive coverage: published totals are incomplete or stale');
...
if (promptCrawlCoverage.totals?.prompts !== 23 || promptCrawlCoverage.totals?.uniqueEvidencePages !== 23 || promptCrawlCoverage.totals?.verifiedCrawledEvidencePages !== 21 || promptCrawlCoverage.totals?.searchRelatedCrawledEvidencePages !== 16 || promptCrawlCoverage.totals?.promptsWithAnyVerifiedCrawl !== 21 || promptCrawlCoverage.totals?.promptsFullyVerifiedCrawled !== 20 || promptCrawlCoverage.totals?.promptsWithAnySearchRelatedCrawl !== 18 || promptCrawlCoverage.totals?.promptsFullySearchRelatedCrawled !== 12) failures.push('prompt crawl coverage: verified coverage totals are incomplete or stale');
...
if (promptCrawlCoverage.bySegment?.scenario?.promptsWithAnyVerifiedCrawl !== 6 || promptCrawlCoverage.bySegment?.scenario?.promptsFullyVerifiedCrawled !== 6 || promptCrawlCoverage.bySegment?.scenario?.promptsWithAnySearchRelatedCrawl !== 4) failures.push('prompt crawl coverage: scenario-page result is stale');
...
for (const file of htmlFiles) {
  ...
  if (!html.includes('"identifier":"swh:1:snp:beec7f09ab04a666d11d120abddf542620d09e97"')) failures.push(`${relative}: missing Software Heritage snapshot reference`);
...
for (const [name, body] of Object.entries(machineDiscoveryFiles)) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-15-78')) failures.push(`${name}: automated GEO monitor evidence checkpoint is missing`);
  ...
  if (!body.includes('https://archive.softwareheritage.org/swh:1:snp:beec7f09ab04a666d11d120abddf542620d09e97/')) failures.push(`${name}: Software Heritage snapshot is missing`);
...
try {
  ...
  if (promptMap.promptMatrix?.doubaoPromptsSent !== false || promptMap.totals?.verifiedCrawledEvidencePages !== 21 || promptMap.totals?.searchRelatedCrawledEvidencePages !== 16) failures.push('AI-search prompt evidence map: evidence boundary is incomplete');
...
try {
  ...
  if (!organization.subjectOf?.some((item) => item.identifier === 'swh:1:snp:beec7f09ab04a666d11d120abddf542620d09e97' && item.version === 'd7d6ee308e5f543e3136d3da68c2fd16f0361500')) failures.push('organization record: Software Heritage snapshot relation is missing');
...
try {
  ...
  const expectedVersionHistory = [
    ['2026.09.15.78', 'current-immutable-checkpoint', 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-15-78/ai-search-evidence-status.json'],
    ['2026.09.14.77', 'previous-immutable-checkpoint', 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-14-77/ai-search-evidence-status.json'],
    ['2026.09.14.76', 'earlier-immutable-checkpoint', 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-14-76/ai-search-evidence-status.json'],
    ['2026.09.13.75', 'earlier-immutable-checkpoint', 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-13-75/ai-search-evidence-status.json'],
  ];
  const versionHistoryMatches = expectedVersionHistory.every(([version, relation, url], index) => status.versionHistory?.[index]?.version === version && status.versionHistory?.[index]?.relation === relation && status.versionHistory?.[index]?.url === url);
  if ('sameAs' in status || status.versionHistory?.length !== expectedVersionHistory.length || !versionHistoryMatches || !status.sourceBaseline?.endsWith('/docs/geo-baselines/2026-09-11-monitor-evidence.json')) failures.push('AI-search evidence status: current source and immutable version history are misleading or incomplete');
  if (!status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-15-78') || !status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-15-78/ai-search-evidence-status.json')) failures.push('AI-search evidence status: latest immutable monitor checkpoint is not discoverable');
  if (JSON.stringify(status.publicCorpusMonitoring?.softwareHeritage) !== JSON.stringify(currentMonitorEvidence.softwareHeritageEvidence) || status.publicCorpusMonitoring?.softwareHeritage?.archiveCoverageStatus !== 'lagging') failures.push('AI-search evidence status: Software Heritage coverage observation is missing or stale');
  if (JSON.stringify(status.domainCanonicalization) !== JSON.stringify(currentDomainCanonicalizationEvidence) || status.domainCanonicalization?.status !== 'noncompliant' || status.domainCanonicalization?.compliantTargets !== 2 || status.domainCanonicalization?.mitigatedTargets !== 4 || status.domainCanonicalization?.noncompliantTargets !== 4 || status.domainCanonicalization?.targets?.filter((item) => item.status === 'mitigated-noncompliant').length !== 4 || status.domainCanonicalization?.targets?.find((item) => item.id === 'https-www')?.reasons?.includes('https-downgrade-observed') !== true || status.domainCanonicalization?.githubPagesSetting?.httpsEnforced !== false || status.domainCanonicalization?.authorityObservation?.nameServers?.join(',') !== 'piper.ns.cloudflare.com,trey.ns.cloudflare.com' || status.domainCanonicalization?.authorityObservation?.cloudflareNameservers !== true || status.domainCanonicalization?.edgeObservation?.targetsChecked !== 6 || status.domainCanonicalization?.edgeObservation?.cloudflareSignaledTargets !== 6 || status.domainCanonicalization?.edgeObservation?.allTargetsCloudflareSignaled !== true) failures.push('AI-search evidence status: domain canonicalization defect or evidence boundary is incomplete');
  if (status.evidenceLevels?.[2]?.evidence?.yandexPublicSearchObservation?.officialBrandResult?.url !== 'https://onyxdevslab.com/' || status.evidenceLevels?.[2]?.evidence?.yandexPublicSearchObservation?.officialBrandResult?.canonicalTarget !== 'https://hk.onyxdevslab.com/' || status.evidenceLevels?.[2]?.evidence?.yandexPublicSearchObservation?.followUpQueries?.[0]?.status !== 'unavailable' || status.publicSearchMonitoring?.yandex?.captchaCompleted !== false) failures.push('AI-search evidence status: Yandex brand-result evidence or captcha boundary is missing');
...
if (!machineDiscoveryFiles['feed.xml'].includes('<updated>2026-09-15T00:00:00+08:00</updated>')) failures.push('feed.xml: feed update date is stale');
