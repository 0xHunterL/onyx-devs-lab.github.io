...
for (const [name, body] of Object.entries(machineDiscoveryFiles)) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-48')) failures.push(`${name}: automated GEO monitor evidence checkpoint is missing`);
...
if (!machineDiscoveryFiles['llms.txt'].includes('2026-09-12T12:31:15.092Z baseline') || !machineDiscoveryFiles['llms.txt'].includes('provider-maintained checkpoint does not prove indexing')) failures.push('llms.txt: current monitor checkpoint lacks its observation-time or evidence boundary');
...
try {
  ...
  if ('sameAs' in status || status.versionHistory?.length !== 3 || status.versionHistory?.[0]?.version !== '2026.09.12.48' || status.versionHistory?.[0]?.relation !== 'current-immutable-checkpoint' || status.versionHistory?.[0]?.url !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-12-48/ai-search-evidence-status.json' || status.versionHistory?.[1]?.version !== '2026.09.12.41' || status.versionHistory?.[1]?.relation !== 'previous-immutable-checkpoint' || status.versionHistory?.[2]?.version !== '2026.09.11.8' || status.versionHistory?.[2]?.relation !== 'earlier-immutable-checkpoint' || !status.sourceBaseline?.endsWith('/docs/geo-baselines/2026-09-11-monitor-evidence.json')) failures.push('AI-search evidence status: current source and immutable version history are misleading or incomplete');
  if (!status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-12-48') || !status.evidenceSources?.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-12-48/ai-search-evidence-status.json')) failures.push('AI-search evidence status: latest immutable monitor checkpoint is not discoverable');
