...
const promptCrawlCoverage = JSON.parse(fs.readFileSync(path.resolve('docs/geo-baselines/2026-09-10-prompt-crawl-coverage.json'), 'utf8'));
...
if (!referralReportSource.includes('appid:\\s*s~virustotalcloud') || !referralReportSource.includes('distinctUserAgents.size < 2') || !referralReportSource.includes("malformedCampaignVisits.push")) failures.push('referral report: link-scanner, coordinated-burst, or malformed-campaign audit is missing');
...
if (promptCrawlCoverage.totals?.prompts !== 20 || promptCrawlCoverage.totals?.uniqueEvidencePages !== 20 || promptCrawlCoverage.totals?.verifiedCrawledEvidencePages !== 8 || promptCrawlCoverage.totals?.searchRelatedCrawledEvidencePages !== 0 || promptCrawlCoverage.totals?.promptsWithAnyVerifiedCrawl !== 10 || promptCrawlCoverage.totals?.promptsFullyVerifiedCrawled !== 6 || promptCrawlCoverage.totals?.promptsWithAnySearchRelatedCrawl !== 0 || promptCrawlCoverage.totals?.promptsFullySearchRelatedCrawled !== 0) failures.push('prompt crawl coverage: verified coverage totals are incomplete or stale');
if (promptCrawlCoverage.promptCoverage?.length !== promptMatrix.prompts.length || !promptMatrix.prompts.every((prompt) => promptCrawlCoverage.promptCoverage.some((entry) => entry.id === prompt.id && entry.segment === prompt.segment))) failures.push('prompt crawl coverage: fixed prompt matrix is not fully represented');
...
if (promptCrawlCoverage.bySegment?.scenario?.promptsWithAnyVerifiedCrawl !== 0) failures.push('prompt crawl coverage: scenario-page gap is not preserved');
...
for (const file of htmlFiles) {
  ...
  if (!html.includes('"identifier":"swh:1:snp:947880d501d459884fefdaf1bc95a9978599727a"')) failures.push(`${relative}: missing Software Heritage snapshot reference`);
  if (!html.includes('"identifier":"swh:1:snp:45a57c93314db28331263c9449dd86ab8519b585"')) failures.push(`${relative}: missing buyer-guide archive reference`);
...
for (const [name, body] of Object.entries(machineDiscoveryFiles)) {
  ...
  if (!body.includes('https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-11-2')) failures.push(`${name}: automated GEO monitor evidence checkpoint is missing`);
  ...
  if (!body.includes('https://archive.softwareheritage.org/swh:1:snp:947880d501d459884fefdaf1bc95a9978599727a/')) failures.push(`${name}: Software Heritage snapshot is missing`);
  ...
  for (const page of ['ai-consulting/', 'ai-custom-development/', 'forward-deployed-engineering/', 'ai-search-geo-evidence/']) if (!body.includes(`https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/${page}`)) failures.push(`${name}: focused buyer-guide page is missing: ${page}`);
  for (const asset of ['CITATION.cff', 'codemeta.json']) if (!body.includes(`https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/${asset}`)) failures.push(`${name}: buyer-guide metadata is missing: ${asset}`);
  if (!body.includes('https://github.com/mixuechu/hong-kong-enterprise-ai-buyers-guide/releases/tag/buyers-guide-geo-evidence-2026-09-11')) failures.push(`${name}: versioned buyer-guide checkpoint is missing`);
  if (!body.includes('https://archive.softwareheritage.org/swh:1:snp:45a57c93314db28331263c9449dd86ab8519b585/')) failures.push(`${name}: buyer-guide archive is missing`);
...
try {
  ...
  if (!jsonFeed.items.some((item) => item.url === 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/tag/geo-monitor-evidence-2026-09-11-2' && item.date_modified === '2026-09-11T00:00:00+08:00')) failures.push('feed.json: automated monitor evidence checkpoint entry is missing or stale');
  ...
  if (!jsonFeed.items.some((item) => item.tags?.includes('independent-archive') && item.url === 'https://archive.softwareheritage.org/swh:1:snp:947880d501d459884fefdaf1bc95a9978599727a/')) failures.push('feed.json: Software Heritage archive item is missing');
  ...
  for (const page of ['ai-consulting/', 'ai-custom-development/', 'forward-deployed-engineering/', 'ai-search-geo-evidence/']) if (!jsonFeed.items.some((item) => item.tags?.includes('provider-maintained-external-source') && item.url === `https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/${page}`)) failures.push(`feed.json: focused buyer-guide item is missing: ${page}`);
  if (!jsonFeed.items.some((item) => item.tags?.includes('provider-maintained-external-source') && item.url === 'https://github.com/mixuechu/hong-kong-enterprise-ai-buyers-guide/releases/tag/buyers-guide-geo-evidence-2026-09-11')) failures.push('feed.json: versioned buyer-guide checkpoint is missing');
...
try {
  ...
  if (!organization.subjectOf?.some((item) => item.identifier === 'swh:1:snp:947880d501d459884fefdaf1bc95a9978599727a' && item.version === 'fe91aae7bb44331aac110650d1af4cfebf6364d3')) failures.push('organization record: Software Heritage snapshot relation is missing');
  ...
  if (!organization.subjectOf?.some((item) => item.url === 'https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/' && item.sameAs === 'https://github.com/mixuechu/hong-kong-enterprise-ai-buyers-guide/releases/tag/buyers-guide-geo-evidence-2026-09-11' && item.hasPart?.length === 4 && item.hasPart.some((part) => part.url === 'https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/ai-search-geo-evidence/') && item.hasPart.every((part) => part.url?.startsWith('https://mixuechu.github.io/hong-kong-enterprise-ai-buyers-guide/')))) failures.push('organization record: focused buyer-guide relations are missing');
  if (!organization.subjectOf?.some((item) => item.identifier === 'swh:1:snp:45a57c93314db28331263c9449dd86ab8519b585' && item.version === 'd490ec3ca459eb4aaccb803e8bb29188763dabee')) failures.push('organization record: buyer-guide archive relation is missing');
...
try {
  ...
  if (status.schemaVersion !== 2 || status.version !== '2026.09.11.2' || status.observedAt !== '2026-09-11T06:22:24Z') failures.push('AI-search evidence status: unexpected schema, version, or observation time');
  if (status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 7) failures.push('AI-search evidence status: current OAI-SearchBot discovery count is stale');
  if (status.sameAs !== 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-11-2/2026-09-11-monitor-evidence.json') failures.push('AI-search evidence status: current versioned release asset is missing');
  ...
  if (status.evidenceLevels?.[1]?.evidence?.verifiedGptBotContentCrawls !== 24 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 7 || status.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotContentCrawls !== 0 || status.evidenceLevels?.[1]?.evidence?.historicallyVerifiedBingbotContentCrawls !== 7) failures.push('AI-search evidence status: crawler evidence is incomplete');
  if (status.evidenceLevels?.[1]?.evidence?.fixedPromptCoverage?.promptsWithAnyVerifiedCrawl !== 10 || status.evidenceLevels?.[1]?.evidence?.fixedPromptCoverage?.promptsFullyVerifiedCrawled !== 6 || status.evidenceLevels?.[1]?.evidence?.fixedPromptCoverage?.promptsWithAnySearchRelatedCrawl !== 0 || status.evidenceLevels?.[1]?.evidence?.fixedPromptCoverage?.scenarioPromptsWithAnyVerifiedCrawl !== 0) failures.push('AI-search evidence status: fixed-prompt crawl coverage is incomplete');
  ...
  if (status.evidenceLevels?.[3]?.evidence?.trackedAttributionRequests !== 52 || status.evidenceLevels?.[3]?.evidence?.suspectedAutomatedTrackedRequests !== 45 || status.evidenceLevels?.[3]?.evidence?.humanUnverifiedTrackedRequests !== 7 || status.evidenceLevels?.[3]?.evidence?.knownLinkScannerTrackedRequests !== 12 || status.evidenceLevels?.[3]?.evidence?.malformedCampaignRequests !== 6 || status.evidenceLevels?.[3]?.evidence?.realAiReferralVisitsObserved !== 0 || status.evidenceLevels?.[3]?.evidence?.latestVerifiedOffsiteReferral?.referrerHost !== 'mixuechu.github.io') failures.push('AI-search evidence status: referral evidence boundary is incomplete');
} catch {
