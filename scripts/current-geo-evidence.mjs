...
export function hasCurrentMonitorLlmsBoundary(body) {
  return body.includes(currentMonitorEvidence.generatedAt)
    ...
    && body.includes('113 requests while Ahrefs content requests remain zero')
    && body.includes('72 known HTML paths and a dynamic 404')
    && body.includes('mitigated but still noncompliant')
    && body.includes('do not prove search reprocessing')
