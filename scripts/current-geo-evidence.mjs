...
export function hasCurrentMonitorLlmsBoundary(body) {
  return body.includes(`${currentMonitorEvidence.generatedAt} production collection`)
    ...
    && body.includes('Provider-verified Ahrefs discovery evidence is now 101 requests while Ahrefs content requests remain zero')
    ...
    && body.includes('Software Heritage now independently preserves revision 40d2c1d in snapshot f3820205')
    ...
    && body.includes('This is independent code-preservation, public-archive, and technical crawler evidence only')
