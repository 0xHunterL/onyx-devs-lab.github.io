...
try {
  ...
  if (monitor.providerVerifiedCrawlerEvidence?.gptBotContentRequests !== 27 || monitor.providerVerifiedCrawlerEvidence?.gptBotDiscoveryFileRequests !== 13 || monitor.providerVerifiedCrawlerEvidence?.distinctVerifiedContentPaths !== 30 || monitor.providerVerifiedCrawlerEvidence?.historicalReclassification?.homepageContentRequestsAdded !== 3 || monitor.providerVerifiedCrawlerEvidence?.historicalReclassification?.machineResourceDiscoveryRequestsAdded !== 10 || monitor.providerVerifiedCrawlerEvidence?.oaiSearchBotDiscoveryFileRequests !== 8 || monitor.providerVerifiedCrawlerEvidence?.oaiSearchBotContentRequests !== 0 || monitor.identityUnverifiedCrawlerEvidence?.bytespiderContentRequestCandidates !== 0 || monitor.identityUnverifiedCrawlerEvidence?.bytespiderDiscoveryFileRequestCandidates !== 0 || monitor.identityUnverifiedCrawlerEvidence?.syntheticBytespiderReleaseChecksExcluded !== 2368 || monitor.identityUnverifiedCrawlerEvidence?.identityStatus !== 'user-agent-only-unverified' || monitor.attributionEvidence?.trackedRequests !== 73 || monitor.attributionEvidence?.suspectedAutomatedRequests !== 61 || monitor.attributionEvidence?.visitorTypeUnverifiedRequests !== 12 || monitor.attributionEvidence?.knownLinkScannerRequests !== 14 || monitor.attributionEvidence?.knownLinkScannerNetworkRequests !== 2 || monitor.attributionEvidence?.internallyInconsistentUserAgentRequests !== 4 || monitor.attributionEvidence?.aiReferrerAttributedRequests !== 0 || monitor.distributionEvidence?.availableSources !== 24 || monitor.distributionEvidence?.unavailableSources !== 0 || monitor.publicSearchRetest?.officialSiteObserved !== false || monitor.publicSearchRetest?.newPublicDiscussionObserved !== false || monitor.doubaoTestStatus !== 'not-run') failures.push('Versioned GEO monitor evidence revision 8: expected evidence structure or boundary is incomplete');
} catch {
