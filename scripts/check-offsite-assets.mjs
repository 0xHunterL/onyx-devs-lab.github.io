...
const monitorEvidenceV68Base = 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-13-68';
const monitorEvidenceV68Assets = [
  {file:'2026-09-11-monitor-evidence.json',sha256:'ba616552b96335fb33ab9f65b3545631fd46fe403345a9716736ff6a81019973'},
  {file:'ai-search-evidence-status.json',sha256:'5e0a6c26637315cc406450e9a169fde04dc1e60cd98440aae398d3df155158bd'},
  {file:'prompt-matrix.json',sha256:'ed5305c7c1221689dd5c522cf305ea7ff73344ef37f016a3e148d5f598351b6f'},
  {file:'ai-search-prompt-evidence-map.json',sha256:'190a3a64d0b80ccac35528b0fbffe9772bf45a23198f93b8cbb09dc3db721474'},
  {file:'2026-09-11-prompt-crawl-coverage.json',sha256:'fbdc860409a7e0a31276c6c3c3f1d581832c4e234468b9db9ec96e945187d79d'},
];
const monitorEvidenceV68Json = {};
for (const asset of monitorEvidenceV68Assets) {
  const raw = await get(`Versioned GEO monitor revision 68 ${asset.file}`, `${monitorEvidenceV68Base}/${asset.file}`, 'application/');
  const digest = createHash('sha256').update(raw).digest('hex');
  if (digest !== asset.sha256) failures.push(`Versioned GEO monitor revision 68 ${asset.file}: SHA-256 mismatch, got ${digest}`);
  try { monitorEvidenceV68Json[asset.file] = JSON.parse(raw); } catch { failures.push(`Versioned GEO monitor revision 68 ${asset.file}: invalid JSON`); }
}
const monitorV68 = monitorEvidenceV68Json['2026-09-11-monitor-evidence.json'];
if (monitorV68 && (monitorV68.publicStatusVersion !== '2026.09.13.68' || monitorV68.generatedAt !== '2026-09-13T05:01:39Z' || monitorV68.latestEvidenceObservedAt !== '2026-09-13T05:01:39Z' || monitorV68.bingPublicSearchEvidence?.provider !== 'Bing Web Search' || monitorV68.bingPublicSearchEvidence?.queries?.[0]?.result !== 'explicit-no-results' || monitorV68.bingPublicSearchEvidence?.queries?.[1]?.result !== 'official-site-not-observed-in-first-page; query-broadened' || monitorV68.bingPublicSearchEvidence?.categoryQueryProtocol?.status !== 'not-run-in-this-check' || monitorV68.googlePublicSearchAttempt?.captchaCompleted !== false || monitorV68.googlePublicSearchAttempt?.resultConclusionDrawn !== false || monitorV68.publicSearchRetest?.reproducibilityStatus !== 'provider-unattributed-historical-observation' || monitorV68.doubaoTestStatus !== 'not-run')) failures.push('Versioned GEO monitor revision 68: provider-attributed public-search evidence or boundary is incomplete');
...
const monitorEvidenceV70Base = 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-13-70';
const monitorEvidenceV70Assets = [
  {file:'2026-09-11-monitor-evidence.json',sha256:'6cf3989108c99ba12cc33c544c63d0bc79078d17e187b8f44949ee2b722e5dc4'},
  {file:'ai-search-evidence-status.json',sha256:'f9f034959f6c1fe9d582cc4adc3b491e11739b89c4c57e09d43970e422e4c5e6'},
  {file:'prompt-matrix.json',sha256:'ed5305c7c1221689dd5c522cf305ea7ff73344ef37f016a3e148d5f598351b6f'},
  {file:'ai-search-prompt-evidence-map.json',sha256:'11744a52081667bdceb9297a3faaafe4fda1b0f153b8c53dc47c6cc9a51b8aba'},
  {file:'2026-09-11-prompt-crawl-coverage.json',sha256:'7b9eb77978732b6425d408bb02cf4c2629daa9693ad2bb448e937df179151c11'},
];
const monitorEvidenceV70Json = {};
for (const asset of monitorEvidenceV70Assets) {
  const raw = await get(`Versioned GEO monitor revision 70 ${asset.file}`, `${monitorEvidenceV70Base}/${asset.file}`, 'application/');
  const digest = createHash('sha256').update(raw).digest('hex');
  if (digest !== asset.sha256) failures.push(`Versioned GEO monitor revision 70 ${asset.file}: SHA-256 mismatch, got ${digest}`);
  try { monitorEvidenceV70Json[asset.file] = JSON.parse(raw); } catch { failures.push(`Versioned GEO monitor revision 70 ${asset.file}: invalid JSON`); }
}
const monitorV70 = monitorEvidenceV70Json['2026-09-11-monitor-evidence.json'];
if (monitorV70 && (monitorV70.publicStatusVersion !== '2026.09.13.70' || monitorV70.generatedAt !== '2026-09-13T06:11:54.521Z' || monitorV70.latestEvidenceObservedAt !== '2026-09-13T06:09:12.547Z' || monitorV70.softwareHeritageEvidence?.archiveCoverageStatus !== 'current' || monitorV70.softwareHeritageEvidence?.snapshotSwhid !== 'swh:1:snp:beec7f09ab04a666d11d120abddf542620d09e97' || monitorV70.softwareHeritageEvidence?.archiveHead !== 'd7d6ee308e5f543e3136d3da68c2fd16f0361500' || monitorV70.softwareHeritageEvidence?.saveRequest?.id !== 2473685 || monitorV70.softwareHeritageEvidence?.saveRequest?.saveTaskStatus !== 'succeeded' || monitorV70.softwareHeritageEvidence?.saveRequest?.visitStatus !== 'full' || monitorV70.providerVerifiedCrawlerEvidence?.ahrefsbotDiscoveryFileRequests !== 102 || monitorV70.providerVerifiedCrawlerEvidence?.ahrefsbotContentRequests !== 0 || monitorV70.distributionEvidence?.publishedItems !== 14 || monitorV70.distributionEvidence?.expectedAfterCurrentCheckpointPublication?.publishedItems !== 15 || monitorV70.distributionEvidence?.expectedAfterCurrentCheckpointPublication?.availableSources !== 44 || monitorV70.doubaoTestStatus !== 'not-run')) failures.push('Versioned GEO monitor revision 70: Software Heritage, self-publication, crawler, or evidence boundary is incomplete');
...
const monitorEvidenceV72Base = 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-13-72';
const monitorEvidenceV72Assets = [
  {file:'2026-09-11-monitor-evidence.json',sha256:'4ece1eb31e6165c20d5f6ccd023123ff4a661712eef9da6e63f7580453884a95'},
  {file:'ai-search-evidence-status.json',sha256:'a0d71bc1ba2234e358a05e56263ac33a840c33fe515465f16f3618702f225382'},
  {file:'prompt-matrix.json',sha256:'ed5305c7c1221689dd5c522cf305ea7ff73344ef37f016a3e148d5f598351b6f'},
  {file:'ai-search-prompt-evidence-map.json',sha256:'28b9c7f47bdc555e4add7e27a11e5bfffa7d1aed41c4f30b7884dd81719524c6'},
  {file:'2026-09-11-prompt-crawl-coverage.json',sha256:'7b9eb77978732b6425d408bb02cf4c2629daa9693ad2bb448e937df179151c11'},
];
const monitorEvidenceV72Json = {};
for (const asset of monitorEvidenceV72Assets) {
  const raw = await get(`Versioned GEO monitor revision 72 ${asset.file}`, `${monitorEvidenceV72Base}/${asset.file}`, 'application/');
  const digest = createHash('sha256').update(raw).digest('hex');
  if (digest !== asset.sha256) failures.push(`Versioned GEO monitor revision 72 ${asset.file}: SHA-256 mismatch, got ${digest}`);
  try { monitorEvidenceV72Json[asset.file] = JSON.parse(raw); } catch { failures.push(`Versioned GEO monitor revision 72 ${asset.file}: invalid JSON`); }
}
const monitorV72 = monitorEvidenceV72Json['2026-09-11-monitor-evidence.json'];
if (monitorV72 && (
  monitorV72.publicStatusVersion !== '2026.09.13.72'
  || monitorV72.generatedAt !== '2026-09-13T09:14:08.977Z'
  || monitorV72.providerVerifiedCrawlerEvidence?.gptBotDiscoveryFileRequests !== 15
  || monitorV72.providerVerifiedCrawlerEvidence?.oaiSearchBotDiscoveryFileRequests !== 13
  || monitorV72.providerVerifiedCrawlerEvidence?.oaiSearchBotContentRequests !== 0
  || monitorV72.providerVerifiedCrawlerEvidence?.ahrefsbotDiscoveryFileRequests !== 105
  || monitorV72.attributionEvidence?.visitorTypeUnverifiedRequests !== 15
  || monitorV72.attributionEvidence?.aiReferrerAttributedRequests !== 0
  || monitorV72.distributionEvidence?.publishedItems !== 16
  || monitorV72.distributionEvidence?.expectedAfterCurrentCheckpointPublication?.publishedItems !== 17
  || monitorV72.distributionEvidence?.expectedAfterCurrentCheckpointPublication?.availableSources !== 48
  || monitorV72.doubaoTestStatus !== 'not-run'
)) failures.push('Versioned GEO monitor revision 72: crawler, attribution, self-publication, or evidence boundary is incomplete');
const statusV72 = monitorEvidenceV72Json['ai-search-evidence-status.json'];
if (statusV72 && (
  statusV72.version !== '2026.09.13.72'
  || statusV72.observedAt !== '2026-09-13T09:14:08.977Z'
  || statusV72.versionHistory?.[0]?.version !== '2026.09.13.72'
  || statusV72.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotDiscoveryFileVisits !== 13
  || statusV72.evidenceLevels?.[1]?.evidence?.verifiedOaiSearchBotContentCrawls !== 0
  || statusV72.evidenceLevels?.[3]?.evidence?.humanUnverifiedTrackedRequests !== 15
  || statusV72.testProtocol?.doubaoPromptsSent !== false
)) failures.push('Versioned GEO monitor revision 72 public status: crawler, attribution, or AI boundary is incomplete');

const monitorEvidenceV73Base = 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-13-73';
const monitorEvidenceV73Assets = [
  {file:'2026-09-11-monitor-evidence.json',sha256:'282afa70c451f151c669ec3e8e6c366a020803a75bf46e552ef7764d402520b8'},
  {file:'ai-search-evidence-status.json',sha256:'90e9e5de2c41d0da92e2c59e5b0c286fcfe5ef5fe82324c35872223ce23b7335'},
  {file:'prompt-matrix.json',sha256:'ed5305c7c1221689dd5c522cf305ea7ff73344ef37f016a3e148d5f598351b6f'},
  {file:'ai-search-prompt-evidence-map.json',sha256:'90a7fde0ed57c5b50f95901f496df58a545e61b6d0d31e312cb9962939cf5aa9'},
  {file:'2026-09-11-prompt-crawl-coverage.json',sha256:'7b9eb77978732b6425d408bb02cf4c2629daa9693ad2bb448e937df179151c11'},
];
const monitorEvidenceV73Json = {};
for (const asset of monitorEvidenceV73Assets) {
  const raw = await get(`Versioned GEO monitor revision 73 ${asset.file}`, `${monitorEvidenceV73Base}/${asset.file}`, 'application/');
  const digest = createHash('sha256').update(raw).digest('hex');
  if (digest !== asset.sha256) failures.push(`Versioned GEO monitor revision 73 ${asset.file}: SHA-256 mismatch, got ${digest}`);
  try { monitorEvidenceV73Json[asset.file] = JSON.parse(raw); } catch { failures.push(`Versioned GEO monitor revision 73 ${asset.file}: invalid JSON`); }
}
const monitorV73 = monitorEvidenceV73Json['2026-09-11-monitor-evidence.json'];
if (monitorV73 && (
  monitorV73.publicStatusVersion !== '2026.09.13.73'
  || monitorV73.generatedAt !== '2026-09-13T09:49:52Z'
  || monitorV73.bingPublicSearchEvidence?.provider !== 'Bing Web Search'
  || monitorV73.bingPublicSearchEvidence?.queries?.length !== 5
  || monitorV73.bingPublicSearchEvidence?.queries?.[0]?.result !== 'official-site-observed-in-first-visible-page'
  || monitorV73.bingPublicSearchEvidence?.queries?.[0]?.visibleOfficialResults?.[1]?.url !== 'https://hk.onyxdevslab.com/zh-hk/'
  || monitorV73.bingPublicSearchEvidence?.queries?.[4]?.result !== 'official-Onyx-result-not-observed-in-visible-first-page'
  || monitorV73.bingPublicSearchEvidence?.previousObservation?.exactMarkerResult !== 'explicit-no-results'
  || monitorV73.distributionEvidence?.publishedItems !== 17
  || monitorV73.distributionEvidence?.expectedAfterCurrentCheckpointPublication?.publishedItems !== 18
  || monitorV73.distributionEvidence?.expectedAfterCurrentCheckpointPublication?.availableSources !== 50
  || monitorV73.doubaoTestStatus !== 'not-run'
)) failures.push('Versioned GEO monitor revision 73: Bing brand, category, self-publication, or evidence boundary is incomplete');
const statusV73 = monitorEvidenceV73Json['ai-search-evidence-status.json'];
if (statusV73 && (
  statusV73.version !== '2026.09.13.73'
  || statusV73.observedAt !== '2026-09-13T09:49:52Z'
  || statusV73.versionHistory?.[0]?.version !== '2026.09.13.73'
  || statusV73.publicSearchChecks?.length !== 10
  || statusV73.evidenceLevels?.[2]?.evidence?.bingPublicSearchObservation?.queries?.[0]?.officialSiteObserved !== true
  || statusV73.evidenceLevels?.[2]?.evidence?.bingPublicSearchObservation?.queries?.[4]?.officialSiteObserved !== false
  || statusV73.evidenceLevels?.[2]?.status !== 'not-verified'
  || statusV73.evidenceLevels?.[3]?.status !== 'not-tested'
...
const monitorEvidenceV76Base = 'https://github.com/0xHunterL/onyx-devs-lab.github.io/releases/download/geo-monitor-evidence-2026-09-14-76';
const monitorEvidenceV76Assets = [
  {file:'2026-09-11-monitor-evidence.json',sha256:'713afe0b53de22cd405de3f25e88368d7372fb0f2b888a2b5f1d7ac5ed364b7d'},
  {file:'ai-search-evidence-status.json',sha256:'a3e320665c0d5b8460ddf7cfc47c72e95b0102552a4b0a4a90111fcfab389a0c'},
  {file:'prompt-matrix.json',sha256:'ed5305c7c1221689dd5c522cf305ea7ff73344ef37f016a3e148d5f598351b6f'},
  {file:'ai-search-prompt-evidence-map.json',sha256:'16ee7d2b1482e3341d6d4d1786b4791361100c9b4be311ba6762a4f3f854ecdc'},
  {file:'2026-09-11-prompt-crawl-coverage.json',sha256:'c35a4d6de60a9593fb33dad223a095614f73673eca6c31e169e2846c4c0ed34c'},
];
const monitorEvidenceV76Json = {};
for (const asset of monitorEvidenceV76Assets) {
  const raw = await get(`Versioned GEO monitor revision 76 ${asset.file}`, `${monitorEvidenceV76Base}/${asset.file}`, 'application/');
  const digest = createHash('sha256').update(raw).digest('hex');
  if (digest !== asset.sha256) failures.push(`Versioned GEO monitor revision 76 ${asset.file}: SHA-256 mismatch, got ${digest}`);
  try { monitorEvidenceV76Json[asset.file] = JSON.parse(raw); } catch { failures.push(`Versioned GEO monitor revision 76 ${asset.file}: invalid JSON`); }
}
const monitorV76 = monitorEvidenceV76Json['2026-09-11-monitor-evidence.json'];
if (monitorV76 && (
  monitorV76.publicStatusVersion !== '2026.09.14.76'
  || monitorV76.generatedAt !== '2026-09-13T17:26:04.446Z'
  || monitorV76.providerVerifiedCrawlerEvidence?.bingbotContentRequests !== 8
  || monitorV76.providerVerifiedCrawlerEvidence?.bingbotDiscoveryFileRequests !== 2
  || monitorV76.providerVerifiedCrawlerEvidence?.ahrefsbotDiscoveryFileRequests !== 113
  || monitorV76.providerVerifiedCrawlerEvidence?.ahrefsbotContentRequests !== 0
  || monitorV76.fixedPromptCoverage?.searchRelatedCrawledEvidencePages !== 16
  || monitorV76.waybackEvidence?.captures !== 98
  || monitorV76.distributionEvidence?.publishedItems !== 21
  || monitorV76.distributionEvidence?.availableSources !== 57
  || monitorV76.distributionEvidence?.expectedAfterCurrentCheckpointPublication?.publishedItems !== 22
  || monitorV76.distributionEvidence?.expectedAfterCurrentCheckpointPublication?.availableSources !== 59
  || monitorV76.doubaoTestStatus !== 'not-run'
)) failures.push('Versioned GEO monitor revision 76: crawler, archive, distribution, or evidence boundary is incomplete');
const statusV76 = monitorEvidenceV76Json['ai-search-evidence-status.json'];
if (statusV76 && (
  statusV76.version !== '2026.09.14.76'
  || statusV76.observedAt !== '2026-09-13T17:26:04.446Z'
  || statusV76.versionHistory?.[0]?.version !== '2026.09.14.76'
  || statusV76.evidenceLevels?.[1]?.evidence?.historicallyVerifiedBingbotContentCrawls !== 8
  || statusV76.evidenceLevels?.[1]?.evidence?.verifiedAhrefsBotDiscoveryFileCrawls !== 113
  || statusV76.evidenceLevels?.[2]?.status !== 'not-verified'
  || statusV76.evidenceLevels?.[3]?.status !== 'not-tested'
